...
namespace KGeoBag
{
  ...
  public:
    ...
    KGRectangle() {}
    KGRectangle(const double& a,
		const double& b,
		const KThreeVector& p0,
		const KThreeVector& n1,
		const KThreeVector& n2);

    KGRectangle(const KThreeVector& p0,
		const KThreeVector& p1,
		const KThreeVector& p2,
    ...
    void SetN1(const KThreeVector& d) { fN1 = d.Unit(); }
    void SetN2(const KThreeVector& d) { fN2 = d.Unit(); }

    double              GetA()  const { return fA; }
    double              GetB()  const { return fB; }
    const KThreeVector& GetP0() const { return fP0; }
    const KThreeVector& GetN1() const { return fN1; }
    const KThreeVector& GetN2() const { return fN2; }
    const KThreeVector  GetN3() const { return fN1.Cross(fN2); }
    const KThreeVector  GetP1() const { return fP0 + fN1*fA; }
    const KThreeVector  GetP2() const { return fP0+fN1*fA+fN2*fB; }
    const KThreeVector  GetP3() const { return fP0 + fN2*fB; }
