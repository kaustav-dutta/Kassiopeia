...
namespace KGeoBag
{
  ...
  public:
    ...
    void SetP1(const KThreeVector& p) { fP1 = p; }
    void SetRadius(double d) { fRadius = d; }
    void SetAxialMeshCount(unsigned int i) { fAxialMeshCount = i; }
    void SetLongitudinalMeshCount(unsigned int i) { fLongitudinalMeshCount = i;}
    void SetLongitudinalMeshPower(double d) { fLongitudinalMeshPower = d; }

    const KThreeVector& GetP0() const { return fP0; }
    const KThreeVector& GetP1() const { return fP1; }
    double GetRadius() const { return fRadius; }
    unsigned int GetAxialMeshCount() const { return fAxialMeshCount; }
    unsigned int GetLongitudinalMeshCount() const { return fLongitudinalMeshCount; }
    double GetLongitudinalMeshPower() const { return fLongitudinalMeshPower; }
