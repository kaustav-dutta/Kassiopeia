...
namespace KGeoBag
{
  ...
  public:
    ...
    void AddCircularPort(double asub[3],
			 double rsub);
    void AddRectangularPort(double asub[3],
			    double length,
			    double width);
    ...
    void SetRMain(double d)     { fRMain = d; }
    void SetNumDiscMain(int i) const { fNumDiscMain = i; }
    void SetPolyMain(int i) const { fPolyMain = i; }

    const double* GetAMain() const { return fAMain; }
    const double* GetBMain() const { return fBMain; }
    double   GetAMain(int i) const { return (i<3) ? fAMain[i] : 0; }
    double   GetBMain(int i) const { return (i<3) ? fBMain[i] : 0; }
    ...
    public:
      ...
      KGPortHousing* GetPortHousing() const { return fPortHousing; }
      void SetPortHousing(KGPortHousing* p) { fPortHousing = p; }

      virtual void ComputeLocalFrame(double *cen,
				     double *x,
				     double *y,
				     double *z) const = 0;
    ...
    public:
      RectangularPort() {}
      RectangularPort(KGPortHousing* portHousing,
		      double       asub[3],
		      double       length,
		      double       width);
      ...
      double GetPortLength()    const { return fPortLength; }

      // discretization parameters
      void     SetXDisc(int i) const { fXDisc = i; }
      void     SetNumDiscSub(int i) const { fNumDiscSub = i; }
      void     SetLengthDisc(int i) const { fLengthDisc = i; }
      void     SetWidthDisc(int i) const { fWidthDisc = i; }
      void     SetBoxLength(double d) const { fBoxLength = d; }
      void     SetBoxWidth(double d) const { fBoxWidth = d; }
    ...
    public:
      CircularPort() {}
      CircularPort(KGPortHousing* portHousing,
		   double       asub[3],
		   double       rsub);
      ...
      double GetLength()       const { return fLength; }
      double GetNorm(int i)  const { return (i<3) ? fNorm[i] : 0; }
    ...
    const KGPortHousing::Port* GetPort(int i) const { return const_cast<const KGPortHousing::Port*>(fPorts.at(i)); }
