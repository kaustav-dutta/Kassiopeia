...
namespace KGeoBag
{
  ...
  public:
    KGExtrudedObject() : fZMin(0.),
			 ...
			 fClosedLoops(false),
			 fBackwards(false) {}
    KGExtrudedObject(double zMin,
		     double zMax,
		     int    nDisc,
		     bool   closedLoops) : fZMin(zMin),
					   fZMax(zMax),
					   fNDisc(nDisc),
					   fDiscretizationPower(2.),
					   fNInnerSegments(0),
    ...
    void SetNDisc(int ndisc)              { fNDisc = ndisc; }
    void Open()                           { fClosedLoops = false; }
    void Close()                          { fClosedLoops = true; }
    void Forwards()                       { fBackwards = false; }
    void Backwards()                      { fBackwards = true; }

    void AddInnerLine(double p1[2],double p2[2]);
    void AddOuterLine(double p1[2],double p2[2]);
    void AddInnerArc(double p1[2],
		     double p2[2],
		     double radius,
		     bool positiveOrientation=true);
    void AddOuterArc(double p1[2],
		     double p2[2],
		     double radius,
		     bool positiveOrientation=true);
    ...
    bool   ClosedLoops()            const { return fClosedLoops; }
    bool   IsBackwards()            const { return fBackwards; }
    ...
    public:
      ...
      static bool comp(const KGExtrudedObject::Line* left,
		       const KGExtrudedObject::Line* right)
      { return (left->GetOrder()<right->GetOrder()) ? true : false; }

      void SetExtruded(KGExtrudedObject* e) { fExtruded = e; }
    ...
    unsigned int GetNOuterSegments() const { return fOuterSegments.size(); }
    unsigned int GetNInnerSegments() const { return fInnerSegments.size(); }
    ...
    static bool CompareTheta(std::vector<double> p1,
			     std::vector<double> p2);

    static bool RayIntersectsLineSeg(const std::vector<double>& p0,
				     const std::vector<double>& s1,
				     const std::vector<double>& s2,
				     std::vector<double>& p_int);

    static bool PointIsInPolygon(std::vector<double> &p1,
