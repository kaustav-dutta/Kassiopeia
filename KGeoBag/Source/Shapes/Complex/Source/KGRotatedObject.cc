...
namespace KGeoBag
{
  ...
  void KGRotatedObject::AddArc(const double p1[2],
			       const double p2[2],
			       const double radius,
  ...
  double KGRotatedObject::DistanceTo(const double* P,double* P_in,double* P_norm) const
  {
	...
	{
	  if (P_in) P_in[j] = P_in1[j];
	  if (P_norm) P_norm[j] = P_norm1[j];
	}
      }
    }
    return distance;
  ...
  KGRotatedObject::Line::Line(KGRotatedObject* rO,
			      const double    p1[2],
			      const double    p2[2])
  {
    fRotated = rO;
  ...
  void KGRotatedObject::Line::Initialize() const
  {
    ...
    // conic section
    fLengthSquared = ((fP2[0]-fP1[0])*(fP2[0]-fP1[0]) +
  ...
  double KGRotatedObject::Line::DistanceTo(const double* P,double* P_in,double* P_norm) const
  {
	...
	P_norm[0] = ((fP1[1]-fP2[1])/(fP2[0]-fP1[0]))/
	  sqrt(1.+((fP1[1]-fP2[1])/(fP2[0]-fP1[0]))*
	       ((fP1[1]-fP2[1])/(fP2[0]-fP1[0])));
	P_norm[1] = 0.;
	P_norm[2] =
	  1./sqrt(1.+((fP1[1]-fP2[1])/
		      (fP2[0]-fP1[0]))*((fP1[1]-fP2[1])/(fP2[0]-fP1[0])));
      }
  ...
  KGRotatedObject::Arc::Arc(KGRotatedObject* rO,
			    const double    p1[2],
			    const double    p2[2],
			    const double    radius,
			    const bool      positiveOrientation)
  ...
  void KGRotatedObject::Arc::Initialize() const
  {
    ...
    fArea = 2.*M_PI*(fCenter[1]*fRadius*(fPhiEnd-fPhiStart) +
		     fRadius*fRadius*(cos(fPhiStart)-cos(fPhiEnd)));

    // fVolume: the volume of the rotated arc.  Hooray calculus!
    fVolume = -fRadius*M_PI/12.*((-3.*(4.*fCenter[1]*fCenter[1]+3.*fRadius*fRadius*fRadius)*cos(fPhiEnd)+fRadius*(fRadius*cos(3.*fPhiEnd)-6.*fCenter[1]*(-2.*fPhiEnd+sin(2.*fPhiEnd)))) - (-3.*(4.*fCenter[1]*fCenter[1]+3.*fRadius*fRadius*fRadius)*cos(fPhiStart)+fRadius*(fRadius*cos(3.*fPhiStart)-6.*fCenter[1]*(-2.*fPhiStart+sin(2.*fPhiStart)))));
  ...
  bool KGRotatedObject::Arc::ContainsPoint(const double* P) const
  {
    ...
    if (arcOpensUp)
    {
      if (r >= fCenter[1]) return false;

      if (sqrt((r-fCenter[1])*(r-fCenter[1]) +
	       (P[2]-fCenter[0])*(P[2]-fCenter[0]))<fRadius)
	return false;
  ...
  bool KGRotatedObject::Arc::AngleIsWithinRange(double phi_test,
						double phi_min,
						double phi_max,
