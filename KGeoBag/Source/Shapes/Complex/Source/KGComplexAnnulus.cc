...
namespace KGeoBag
{
  ...
  void KGComplexAnnulus::AddRing(double asub[2],
				      double rsub)
  {
  ...
  bool KGComplexAnnulus::ContainsPoint(const double* P) const
  {
	...
	for(unsigned int i = 0; i<fRings.size(); i++){
		if(fRings.at(i)->ContainsPoint(P)) return false;
	}
  ...
  KGComplexAnnulus::Ring::Ring(KGComplexAnnulus* complexAnnulus,
						  double       ASub[2],
						  double       RSub)
    : Ring(complexAnnulus)
  {
  ...
  bool KGComplexAnnulus::Ring::ContainsPoint(const double* P) const
  {
    ...
    double epsilon = 1e-7;
    if(fabs(P_loc[2]) > epsilon) return false;
