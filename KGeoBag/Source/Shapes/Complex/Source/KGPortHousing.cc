...
namespace KGeoBag
{
  ...
  void KGPortHousing::AddCircularPort(double asub[3],
				      double rsub)
  {
  ...
  void KGPortHousing::AddRectangularPort(double asub[3],
					 double length,
					 double width)
  {
  ...
  double KGPortHousing::DistanceTo(const double* P,double* P_in,double* P_norm) const
  {
    ...
    for (unsigned int i=0;i<fPorts.size();i++)
    {
	  ...
	  (fPorts.at(i)->ContainsPoint(P_main) && pointIsOnMainCylinder))
      {
  ...
  KGPortHousing::RectangularPort::RectangularPort(KGPortHousing* portHousing,
						  double       asub[3],
						  double       length,
						  double       width)
    : Port(portHousing)
  ...
  {
    if (fCoordTransform) delete fCoordTransform;
  }
  ...
  void KGPortHousing::RectangularPort::Initialize()
  {
    ...
    fPortLength = sqrt((fCen[0]-fASub[0])*(fCen[0]-fASub[0]) +
		       (fCen[1]-fASub[1])*(fCen[1]-fASub[1]) +
  ...
  void KGPortHousing::RectangularPort::ComputeLocalFrame(double *cen,
							 double *x,
							 double *y,
							 double *z) const
  {
  ...
  double KGPortHousing::RectangularPort::DistanceTo(const double* P,
						    double* P_in,
						    double* P_norm) const
  {
    double P_loc[3];
	...
	P_tmp[1] = P_loc[1];
	if (P_tmp[1] < -fLength/2.) P_tmp[1] = -fLength/2.;
	else if (P_tmp[1] > fLength/2.) P_tmp[1] = fLength/2.;
	P_tmp[2] = P_loc[2];
	if (P_tmp[2] < rMain*cos(theta)) P_tmp[2] = rMain*cos(theta);

	if (P_in)
	  fCoordTransform->ConvertToGlobalCoords(P_tmp,P_in,false);

	return sqrt((P_tmp[0]-P_loc[0])*(P_tmp[0]-P_loc[0]) +
		    (P_tmp[1]-P_loc[1])*(P_tmp[1]-P_loc[1]) +
	...
	P_tmp[0] = P_loc[0];
	if (P_tmp[0]<-fWidth/2.) P_tmp[0] = -fWidth/2.;
	else if (P_tmp[0]>fWidth/2.) P_tmp[0] = fWidth/2.;
	...
	if (fabs(fabs(P_loc[0]) - fWidth/2.) > fabs(fabs(P_loc[1]) - fLength/2.))
	{
	  ...
	  P_n[1] = (P_loc[1] < 0.) ? 1. : -1.;
	  P_tmp[0] = P_loc[0];
	  if (P_loc[0] > fWidth/2.) P_tmp[0] = fWidth/2.;
	  else if (P_loc[0] < -fWidth/2.) P_tmp[0] = -fWidth/2.;
	  P_tmp[1] = (P_loc[1] < 0.) ? -fLength/2. : fLength/2.;
	}
	else
	{
	  ...
	  P_tmp[0] = (P_loc[0] < 0.) ? -fWidth/2. : fWidth/2.;
	  P_tmp[1] = P_loc[1];
	  if (P_loc[1] > fLength/2.) P_tmp[1] = fLength/2.;
	  else if (P_loc[1] < -fLength/2.) P_tmp[1] = -fLength/2.;
	}
      }
      else
      {
  ...
  KGPortHousing::CircularPort::CircularPort(KGPortHousing* portHousing,
					    double       asub[3],
					    double       rsub)
  : Port(portHousing)
  {
  ...
  void KGPortHousing::CircularPort::ComputeLocalFrame(double *cen,
						      double *x,
						      double *y,
						      double *z) const
  {
  ...
  double KGPortHousing::CircularPort::DistanceTo(const double* P,
						 double* P_in,
						 double* P_norm) const
  {
