...
namespace KGeoBag
{
    ...
    double
    KGMeshRectangle::NearestDistance(const KThreeVector& aPoint) const
    {
    ...
    KThreeVector
    KGMeshRectangle::NearestPoint(const KThreeVector& aPoint) const
    {
        ...
        if( x_coord > fA ){x_coord = fA;};
        if( x_coord < 0 ){x_coord = 0;};

        if( y_coord > fB ){y_coord = fB;};
    ...
    KThreeVector
    KGMeshRectangle::NearestNormal(const KThreeVector& /*aPoint*/) const
    {
        return (fN1.Cross(fN2)).Unit();
    }

    bool
    KGMeshRectangle::NearestIntersection(const KThreeVector& aStart, const KThreeVector& anEnd, KThreeVector& anIntersection) const
    {
        ...
        if( (projection.X() <= fA) && (projection.X() >= 0) &&
            (projection.Y() <= fB) && (projection.Y() >= 0) )
        {
    ...
    KGMeshRectangle::GetPointCloud() const
    {
    ...
    void
    KGMeshRectangle::GetEdge(KThreeVector& start, KThreeVector& end, unsigned int index) const
    {
        if(index == 0)
        {
            start = fP0;
