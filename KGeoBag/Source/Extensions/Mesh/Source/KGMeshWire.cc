...
namespace KGeoBag
{
    ...
    double
    KGMeshWire::NearestDistance(const KThreeVector& aPoint) const
    {
        ...
        //nearest point is p0
        if(dot < 0. ){return del.Magnitude();}
        //nearest point is p1
        if(dot > 1.){  return (aPoint - fP1).Magnitude(); }
    ...
    KThreeVector
    KGMeshWire::NearestPoint(const KThreeVector& aPoint) const
    {
        KThreeVector del = aPoint - fP0;
        KThreeVector diff = fP1 - fP0;
        double len = diff.Magnitude();
    ...
    KThreeVector
    KGMeshWire::NearestNormal(const KThreeVector& aPoint) const
    {
        KThreeVector del = aPoint - fP0;
        KThreeVector diff = fP1 - fP0;
        double len = diff.Magnitude();
    ...
    double
    KGMeshWire::ClosestApproach(const KThreeVector& aStart, const KThreeVector& anEnd) const
    {
        ...
        if(std::fabs(det) >= 1e-12)
        {
            ...
            if( s <= 1. )
            {
                if( s>=0. ){ Ps = fP0 + s*fDiff; } //Ps is on this line segment
                else{Ps = fP0; } //Ps is below fP1
            }
            else{ Ps = fP1;}//Ps is above fP2

            if( t <= 1. )
            {
                if( t >= 0. ){Qt = aStart + t*tDiff;} //Qt is on line segment under test
                else{Qt = aStart;}//Qt is below aStart
            }
            else{Qt = anEnd;}//Qt is above anEnd
    ...
    bool
    KGMeshWire::NearestIntersection(const KThreeVector& aStart, const KThreeVector& anEnd, KThreeVector& anIntersection) const
    {
    ...
    KGMeshWire::GetPointCloud() const
    {
    ...
    void
    KGMeshWire::GetEdge(KThreeVector& start, KThreeVector& end, unsigned int /*index*/) const
    {
        start = fP0;
        end = fP1;
    }
