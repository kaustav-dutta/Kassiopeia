...
void
KGNavigableMeshIntersectionFinder::SetLineSegment(const KThreeVector& start, const KThreeVector& end)
{
    fStartPoint = start;
    fEndPoint = end;
...
bool
KGNavigableMeshIntersectionFinder::HasIntersectionWithMesh() const
{
...
void
KGNavigableMeshIntersectionFinder::GetIntersectedMeshElements( std::vector< const KGNavigableMeshElement* >* intersected_mesh_elements) const
{
    *intersected_mesh_elements = fIntersectedElements;
}

void
KGNavigableMeshIntersectionFinder::GetIntersections( std::vector< KThreeVector >* intersections) const
{
...
void
KGNavigableMeshIntersectionFinder::NearestPointOnLineSegment(const KThreeVector& aPoint, KThreeVector& aNearest, double& t) const
{
...
double
KGNavigableMeshIntersectionFinder::LineSegmentDistanceToPoint(const KThreeVector& aPoint) const
{
...
bool
KGNavigableMeshIntersectionFinder::LineSegmentIntersectsCube(const KGCube<KGMESH_DIM>* cube, double& distance) const
{
    distance = 1e30;
...
void
KGNavigableMeshIntersectionFinder::ApplyAction(KGMeshNavigationNode* node)
{
    fHaveIntersection = false;
                    ...
                    for(unsigned int i=0; i<n_elem; i++)
                    {
                        ...
                        if(check_it == fCheckedElements.end() )
                        {
                            ...
                            if(lin_seg_to_bball < bball.GetRadius())
                            {
                                ...
                                bool inter = current_elem->GetMeshElement()->NearestIntersection(fStartPoint, fEndPoint, anIntersection);
                                if(inter)
                                {
