...
void
KGNavigableMeshFirstIntersectionFinder::SortOctreeNodes(unsigned int n_nodes, std::pair< KGMeshNavigationNode*, double >* nodes)
{
    ...
    #define SWAP(x,y) if (nodes[y].second > nodes[x].second) { tmp = nodes[x]; nodes[x] = nodes[y]; nodes[y] = tmp; }
...
void
KGNavigableMeshFirstIntersectionFinder::SetLineSegment(const KThreeVector& start, const KThreeVector& end)
{
    fStartPoint = start;
    fEndPoint = end;
...
bool
KGNavigableMeshFirstIntersectionFinder::HasIntersectionWithMesh() const
{
    return fHaveIntersection;
};

KThreeVector
KGNavigableMeshFirstIntersectionFinder::GetIntersection() const
{
...
void
KGNavigableMeshFirstIntersectionFinder::NearestPointOnLineSegment(const KThreeVector& aPoint, KThreeVector& aNearest, double& t) const
{
    t = ((aPoint - fStartPoint)*fDirection);
    if(t < 0.){aNearest = fStartPoint; t=0; return;}
    if(t > fLength){aNearest = fEndPoint; t=fLength; return;}
    aNearest = fStartPoint + t*fDirection;
}

double
KGNavigableMeshFirstIntersectionFinder::LineSegmentDistanceToPoint(const KThreeVector& aPoint) const
{
...
bool
KGNavigableMeshFirstIntersectionFinder::LineSegmentIntersectsCube(const KGCube<KGMESH_DIM>& cube, double& distance) const
{
        ...
        do
        {
            ...
            if( fTempNode->HasChildren() )
            {
                ...
                for(unsigned int i=0; i<8; i++) //always 8 chilren in octree
                {
                    ...
                    {
                        fOrderedChildren[n_intersected_children] = std::pair< KGMeshNavigationNode*, double>(child, dist);
            ...
            else
            {
                ...
                if(element_list != nullptr && element_list->GetSize() != 0)
                {
                    ...
                    for(unsigned int i=0; i<n_elem; i++)
                    {
                        ...
                        {
                            bool inter = fContainer->GetElement(element_list->GetID(i))->GetMeshElement()->NearestIntersection(fStartPoint, fEndPoint, anIntersection);
                            if(inter)
                            {
