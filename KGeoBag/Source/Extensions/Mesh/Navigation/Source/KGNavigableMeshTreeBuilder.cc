...
void
KGNavigableMeshTreeBuilder::SetNavigableMeshElementContainer(KGNavigableMeshElementContainer* container)
{
    fContainer = container;
}

KGNavigableMeshElementContainer*
KGNavigableMeshTreeBuilder::GetNavigableMeshElementContainer()
{
    return fContainer;
}

void
KGNavigableMeshTreeBuilder::SetTree(KGNavigableMeshTree* tree)
{
    fTree = tree;
}

KGNavigableMeshTree*
KGNavigableMeshTreeBuilder::GetTree()
{
    return fTree;
}

void
KGNavigableMeshTreeBuilder::ConstructTree()
{
    ConstructRootNode();
...
void
KGNavigableMeshTreeBuilder::ConstructRootNode()
{
    ...
    if(bball_r_min == 0.0){bball_r_min = KGBBALL_EPS;};
    ...
    *world_volume = boundary_calc.GetMinimalBoundingCube();
    ...
    std::cout<<"estimated world cube center is ("<<fWorldCenter[0]<<", "<<fWorldCenter[1]<<", "<<fWorldCenter[2]<<")"<<std::endl;
    ...
    for(unsigned int i=0; i<KGMESH_DIM; i++){ dim[i] = 2;}; //we use an oct-tree in 3d
...
void
KGNavigableMeshTreeBuilder::PerformSpatialSubdivision()
{
