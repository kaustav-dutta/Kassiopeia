    ...
    public:
        ...
        void ApplyAction(KGMeshNavigationNode* node) override
        {
            if(node != nullptr)
            {
                ...
                double length = KGObjectRetriever< KGMeshNavigationNodeObjects, KGCube<KGMESH_DIM> >::GetNodeObject(node)->GetLength();
                fLevelNodeSize[level] = length;

                if(node->GetID() == 0 )//root node
                {
                    KGCube<KGMESH_DIM>* world_cube = KGObjectRetriever< KGMeshNavigationNodeObjects, KGCube<KGMESH_DIM> >::GetNodeObject(node);
        ...
        void SetNElements(unsigned int n){fNElements = n;};
        ...
        std::string GetStatistics()
        {
            ...
            ss<<"Region center at ("<<fWorldCenter[0]<<", "<<fWorldCenter[1]<<", "<<fWorldCenter[2]<<") and side length: "<<fWorldLength<<"\n";
            ...
            if(fNLevelNodes.size() > 0)
            {
                ...
                {
                    ss <<i<<" / "<<fNLevelNodes[i]<<" / "<<fNLevelLeafNodes[i]<<" / "<<fNLevelElements[i]<<" / "<<fLevelMaxNodeElements[i]<<"\n";
                }
                unsigned int n = fNLevelNodes.size()-1;
                ss <<n<<" / "<<fNLevelNodes[n]<<" / "<<fNLevelLeafNodes[n]<<" / "<<fNLevelElements[n]<<" / "<<fLevelMaxNodeElements[n];
            }
            else
            {
