...
namespace KGeoBag
{
    void KGLinearWireGridMesher::VisitWrappedSurface(KGWrappedSurface< KGLinearWireGrid >* linearWireGridSurface)
    {
        ...
        const unsigned int nWires = static_cast<unsigned int>(linearWireGridSurface->GetObject()->GetR()/linearWireGridSurface->GetObject()->GetPitch());
        ...
        for( unsigned int nWire=1; nWire<=nWires; nWire++ )
        {
            ...
            KGComplexMesher::DiscretizeInterval( xLengthIt, xDiscIt, linearWireGridSurface->GetObject()->GetNDiscPower(), xSegmentsIt );
            ...
            for( unsigned int xIt=0; xIt<xSegmentsIt.size(); xIt++ )
            {
                ...
                if (xSegmentsIt[xIt] < minimumSegment) minimumSegment = xSegmentsIt[xIt];
