...
namespace KGeoBag
{
    void KGPortHousingSurfaceMesher::VisitWrappedSurface( KGWrappedSurface< KGPortHousing >* portHousingSurface )
    {
        ...
        for( unsigned int i = 0; i < fPortHousing->GetNPorts(); i++ )
        {
            ...
            phi[ i ] = acos( 1. - v->GetBoxLength() * v->GetBoxLength() / (2. * fPortHousing->GetRMain() * fPortHousing->GetRMain()) );
        }
    ...
    void KGPortHousingSurfaceMesher::ComputeEnclosingBoxLengths( std::vector< double >& theta, std::vector< double >& phi, std::vector< double >& mid, std::vector< double >& width )
    {
        ...
        {
            std::stringstream s;
            s << "In order to properly match boundaries between the valves of this port, the variable PortHousing::fPortHousing->GetPolyMain() has been modified from " << fPortHousing->GetPolyMain() << " to " << tmpPoly << ".";
    ...
    void KGPortHousingSurfaceMesher::RectangularPortDiscretizer::DiscretizePort( const KGPortHousing::RectangularPort* rectangularPort )
    {
        ...
        DiscretizeInterval( 2. * (rectangularPort->GetBoxWidth() - rectangularPort->GetWidth()), 2. * rectangularPort->GetXDisc(), 2., xlen );
        DiscretizeInterval( 2. * (rectangularPort->GetBoxLength() - rectangularPort->GetLength()), 2. * rectangularPort->GetXDisc(), 2., ylen );
        ...
        for( int i = 1; i <= rectangularPort->GetXDisc(); i++ )
        {
            for( int j = 0; j < 2. * (rectangularPort->GetLengthDisc() + rectangularPort->GetWidthDisc()); j++ )
            {
                ...
                BoundingBoxCoord( (j + 1) % (2 * (rectangularPort->GetLengthDisc() + rectangularPort->GetWidthDisc())), ylen[ i - 1 ], xlen[ i - 1 ], p1_loc );
                BoundingBoxCoord( j, ylen[ i ], xlen[ i ], p2_loc );
                BoundingBoxCoord( (j + 1) % (2 * (rectangularPort->GetLengthDisc() + rectangularPort->GetWidthDisc())), ylen[ i ], xlen[ i ], p3_loc );
    ...
    void KGPortHousingSurfaceMesher::RectangularPortDiscretizer::PowerDistBoxCoord( int i, double length, double width, double *xyz )
    {
    ...
    void KGPortHousingSurfaceMesher::RectangularPortDiscretizer::PolygonBoxCoord( int i, double length, double width, double *xyz )
    {
        ...
        i = (i + 3 * fRectangularPort->GetLengthDisc() / 2 + 2 * fRectangularPort->GetWidthDisc()) % (2 * fRectangularPort->GetLengthDisc() + 2 * fRectangularPort->GetWidthDisc());
        ...
        else
        {
            if( i != (2. * fRectangularPort->GetWidthDisc() + 2. * fRectangularPort->GetLengthDisc()) )
                for( int j = 1; j <= ((2. * fRectangularPort->GetWidthDisc() + 2. * fRectangularPort->GetLengthDisc()) - i); j++ )
                    tmp += len * cos( phi * (2 * j - 1) );
            xyz[ 0 ] = width / 2.;
            xyz[ 1 ] = -tmp;
        }
    ...
    void KGPortHousingSurfaceMesher::RectangularPortDiscretizer::BoundingBoxCoord( int i, double length, double width, double *xyz )
    {
        // For a given i, i < 2.*fRectangularPort->GetLengthDisc() + 2.*fRectangularPort->GetWidthDisc(), returns xyz coordinate
        // on a bounding box of dimensions <length>, <width>.

        double ratio = (length - fRectangularPort->GetLength()) / (fRectangularPort->GetBoxLength() - fRectangularPort->GetLength()); // 0 at port, 1 at edge
    ...
    void KGPortHousingSurfaceMesher::CircularPortDiscretizer::DiscretizePort( const KGPortHousing::CircularPort* circularPort )
    {
        ...
        for( int i = 0; i < circularPort->GetXDisc(); i++ )
        {
            ...
            for( int j = 1; j <= circularPort->GetPolySub(); j++ )
            {
                ...
                if( j < circularPort->GetPolySub() )
                    z_high[ j ] = dz[ i ] + rMain * sqrt( 1. - (circularPort->GetRSub() * circularPort->GetRSub() * sine2 / (rMain * rMain)) * alpha[ i ] );
        ...
        DiscretizeInterval( 2. * (circularPort->GetBoxLength() / 2. - circularPort->GetRSub()), 2 * circularPort->GetXDisc(), 2., dz );
        ...
        for( int i = 0; i < circularPort->GetXDisc(); i++ )
        {
            ...
            for( int j = 1; j <= circularPort->GetPolySub(); j++ )
            {
                ...
                // necessary
                double tmp1 = sqrt( (p0_loc[ 0 ] - p3_loc[ 0 ]) * (p0_loc[ 0 ] - p3_loc[ 0 ]) + (p0_loc[ 1 ] - p3_loc[ 1 ]) * (p0_loc[ 1 ] - p3_loc[ 1 ]) + (p0_loc[ 2 ] - p3_loc[ 2 ]) * (p0_loc[ 2 ] - p3_loc[ 2 ]) );

                double tmp2 = sqrt( (p1_loc[ 0 ] - p2_loc[ 0 ]) * (p1_loc[ 0 ] - p2_loc[ 0 ]) + (p1_loc[ 1 ] - p2_loc[ 1 ]) * (p1_loc[ 1 ] - p2_loc[ 1 ]) + (p1_loc[ 2 ] - p2_loc[ 2 ]) * (p1_loc[ 2 ] - p2_loc[ 2 ]) );
                if( tmp2 > tmp1 )
                {
                    double tmp3, tmp4;
                    for( int k = 0; k < 3; k++ )
    ...
    double KGPortHousingSurfaceMesher::CircularPortDiscretizer::Rect_theta( double /*r*/, int i )
    {
        ...
        if( i > 7. * fCircularPort->GetPolySub() / 8. || i < fCircularPort->GetPolySub() / 8. || (i > 3. * fCircularPort->GetPolySub() / 8. && i < 5. * fCircularPort->GetPolySub() / 8.) )
        {
    ...
    double KGPortHousingSurfaceMesher::CircularPortDiscretizer::Transition_theta( double r, int i )
    {
        ...
        double ratio = (r - fCircularPort->GetRSub()) / (fCircularPort->GetBoxLength() / 2. - fCircularPort->GetRSub()); // 0 at the intersection,
    ...
    void KGPortHousingSurfaceMesher::CircularPortDiscretizer::Transition_coord( double r, double theta, double p[ 3 ] )
    {
        ...
        p[ 2 ] = sqrt( fCircularPort->GetPortHousing()->GetRMain() * fCircularPort->GetPortHousing()->GetRMain() - p[ 1 ] * p[ 1 ] );
    }
