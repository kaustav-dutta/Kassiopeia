...
namespace KGeoBag
{
    ...
    void KGExtrudedSurfaceMesher::Discretize( KGExtrudedObject* object )
    {
        ...
        while( !discretizationIsOk )
        {
            ...
            for( unsigned int i = 0; i < fExtrudedObject->GetNOuterSegments(); i++ )
            {
                ...
                if( totalOuterLength > totalOuterLength )
                    nSegments = (int) (nDisc * totalOuterLength / totalInnerLength * fExtrudedObject->GetOuterSegment( i )->GetLength() / totalOuterLength);
                else
            ...
            for( unsigned int i = 0; i < fExtrudedObject->GetNInnerSegments(); i++ )
            {
                ...
                if( totalInnerLength > totalOuterLength )
                    nSegments = (int) (nDisc * totalInnerLength / totalOuterLength * fExtrudedObject->GetInnerSegment( i )->GetLength() / totalInnerLength);
                else
                ...
                if( fExtrudedObject->GetNOuterSegments() > i )
                    if( fExtrudedObject->GetInnerSegment( i )->GetNDisc() == fExtrudedObject->GetOuterSegment( i )->GetNDisc() )
                    {
            ...
            if( loopCounter > 999 )
            {
                ...
                std::stringstream s;
                s << "This extruded surface has undergone " << loopCounter << " iterations, and the boundaries have not yet been simultaneously discretized.  This algorithm probably needs to be made more general!";
        ...
        {
            std::stringstream s;
            s << "In order to properly match boundaries between the inner and outer loops of this surface, the variable ExtrudedSurface::fNDisc has been modified from " << fExtrudedObject->GetNDisc() << " to " << nDisc << ".";
    ...
    void KGExtrudedSurfaceMesher::DiscretizeSegment( const KGExtrudedObject::Line* line, const unsigned int nDisc, std::vector< std::vector< double > >& coords, unsigned int& counter )
    {
        ...
        double n1[ 3 ] =
        { line->GetP2( 0 ) - line->GetP1( 0 ), line->GetP2( 1 ) - line->GetP1( 1 ), 0 };
        ...
        double p0[ 3 ] =
        { line->GetP1( 0 ), line->GetP1( 1 ), fExtrudedObject->GetZMin() };
        ...
        RefineAndAddElement( r, nDisc, fExtrudedObject->GetDiscretizationPower(), nDisc, fExtrudedObject->GetDiscretizationPower() );
    }

    //____________________________________________________________________________

    void KGExtrudedSurfaceMesher::DiscretizeSegment( const KGExtrudedObject::Arc* arc, const unsigned int nDisc, std::vector< std::vector< double > >& coords, unsigned int& counter )
    {
        double z_len = fExtrudedObject->GetZMax() - fExtrudedObject->GetZMin();
        ...
        for( int i = 0; i < nDisc_xy - 1; i++ )
        {
            ...
            RefineAndAddElement( r, 1, fExtrudedObject->GetDiscretizationPower(), z_len / xy_len, fExtrudedObject->GetDiscretizationPower() );
        }
    ...
    void KGExtrudedSurfaceMesher::DiscretizeEnclosedEnds( std::vector< std::vector< double > >& iCoords, std::vector< std::vector< double > >& oCoords, unsigned int nDisc )
    {
        ...
        for( unsigned int i = 0; i < nStrips; i++ )
        {
            ...
            double low_unit[ 2 ] =
            { outer_low[ 0 ] - inner_low[ 0 ], outer_low[ 1 ] - inner_low[ 1 ] };

            double high_unit[ 2 ] =
            { outer_high[ 0 ] - inner_high[ 0 ], outer_high[ 1 ] - inner_high[ 1 ] };

            double low_dist = sqrt( (outer_low[ 0 ] - inner_low[ 0 ]) * (outer_low[ 0 ] - inner_low[ 0 ]) + (outer_low[ 1 ] - inner_low[ 1 ]) * (outer_low[ 1 ] - inner_low[ 1 ]) );

            double high_dist = sqrt( (outer_high[ 0 ] - inner_high[ 0 ]) * (outer_high[ 0 ] - inner_high[ 0 ]) + (outer_high[ 1 ] - inner_high[ 1 ]) * (outer_high[ 1 ] - inner_high[ 1 ]) );
            ...
            double apprxRadLow = fabs( sqrt( outer_low[ 0 ] * outer_low[ 0 ] + outer_low[ 1 ] * outer_low[ 1 ] ) - sqrt( inner_low[ 0 ] * inner_low[ 0 ] + inner_low[ 1 ] * inner_low[ 1 ] ) );
            double apprxRadHigh = fabs( sqrt( outer_high[ 0 ] * outer_high[ 0 ] + outer_high[ 1 ] * outer_high[ 1 ] ) - sqrt( inner_high[ 0 ] * inner_high[ 0 ] + inner_high[ 1 ] * inner_high[ 1 ] ) );
            double apprxRad = (apprxRadLow < apprxRadHigh ? apprxRadLow : apprxRadHigh);
            int n_disc = 6. * nDisc * apprxRad / (2. * M_PI * sqrt( outer_low[ 0 ] * outer_low[ 0 ] + outer_low[ 1 ] * outer_low[ 1 ] ));

            if( n_disc < 3 )
                n_disc += 3;

            std::vector< double > tmp_low( n_disc, 0 );
            DiscretizeInterval( low_dist, n_disc,
            // GetDiscretizationPower(),
                    1.3, tmp_low );
            std::vector< double > tmp_high( n_disc, 0 );
            DiscretizeInterval( high_dist, n_disc,
            // GetDiscretizationPower(),
                    1.3, tmp_high );

            double i_l[ 2 ] =
            { inner_low[ 0 ], inner_low[ 1 ] };
            double i_h[ 2 ] =
            { inner_high[ 0 ], inner_high[ 1 ] };
            double o_l[ 2 ], o_h[ 2 ];
            for( int j = 0; j < 2; j++ )
            {
