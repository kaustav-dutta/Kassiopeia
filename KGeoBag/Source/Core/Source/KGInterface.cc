...
namespace KGeoBag
{
    ...
    void KGInterface::RetrieveSurfacesByPath( vector< KGSurface* >& anAccumulator, KGSpace* aNode, string aPath )
    {
        ...
        if( tNestPos == string::npos )
        {
            if( tHead.find_first_of( sTag ) == 0 )
            {
                ...
                coremsg_debug( "  retrieving final surfaces for tag <" << tTag << "> with recursion <" << tRecursion << ">" << eom );
    ...
    void KGInterface::RetrieveSpacesByPath( vector< KGSpace* >& anAccumulator, KGSpace* aNode, string aPath )
    {
        ...
        if( tNestPos == string::npos )
        {
            if( tHead.find_first_of( sTag ) == 0 )
            {
                ...
                coremsg_debug( "  retrieving final spaces for tag <" << tTag << "> with recursion <" << tRecursion << ">" << eom );
    ...
    void KGInterface::RetrieveSurfacesByName( vector< KGSurface* >& anAccumulator, KGSpace* aNode, string aName )
    {
        ...
        for( tBoundaryIt = aNode->GetBoundaries()->begin(); tBoundaryIt != aNode->GetBoundaries()->end(); tBoundaryIt++ )
        {
            ...
            {
                coremsg_debug( "    boundary surface of <" << aNode->GetName() << "> has matching name <" << aName << ">" << eom )
        ...
        for( tSurfaceIt = aNode->GetChildSurfaces()->begin(); tSurfaceIt != aNode->GetChildSurfaces()->end(); tSurfaceIt++ )
        {
            tSurface = *tSurfaceIt;
            if( tSurface->HasName( aName ) )
            {
                coremsg_debug( "    child surface of <" << aNode->GetName() << "> has matching name <" << aName << ">" << eom )
    ...
    void KGInterface::RetrieveSurfacesByTag( vector< KGSurface* >& anAccumulator, KGSpace* aNode, string aTag, int aDepth )
    {
        ...
        for( tBoundaryIt = aNode->GetBoundaries()->begin(); tBoundaryIt != aNode->GetBoundaries()->end(); tBoundaryIt++ )
        {
            ...
            {
                coremsg_debug( "    boundary surface of <" << aNode->GetName() << "> has matching tag <" << aTag << ">" << eom )
                anAccumulator.push_back( tBoundary );
            }
        }
