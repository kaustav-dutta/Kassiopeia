...
namespace KGeoBag
{
    ...
    void KGVTKAxialMeshPainter::Write()
    {
        if( fWriteEnabled == true )
        {
            ...
            vismsg( eNormal ) << "vtk mesh painter <" << GetName() << "> is writing <" << fPolyData->GetNumberOfCells() << "> cells to file <" << tFileName << ">" << eom;
    ...
    void KGVTKAxialMeshPainter::PaintElements()
    {
        ...
        for( KGAxialMeshElementCIt elementIt = fCurrentElements->begin(); elementIt != fCurrentElements->end(); ++elementIt )
        {
            if( KGAxialMeshLoop* tLoop = dynamic_cast< KGAxialMeshLoop* >( *elementIt ) )
            {
                for( unsigned int tIndex = 0; tIndex < fArcCount; tIndex++ )
                {
                    ...
                    tThisThisPoint = fCurrentOrigin + tLoop->GetP0().Y() * (cos( tThisPhi ) * fCurrentXAxis + sin( tThisPhi ) * fCurrentYAxis) + tLoop->GetP0().X() * fCurrentZAxis;
                    tThisNextPoint = fCurrentOrigin + tLoop->GetP0().Y() * (cos( tNextPhi ) * fCurrentXAxis + sin( tNextPhi ) * fCurrentYAxis) + tLoop->GetP0().X() * fCurrentZAxis;
                    tNextThisPoint = fCurrentOrigin + tLoop->GetP1().Y() * (cos( tThisPhi ) * fCurrentXAxis + sin( tThisPhi ) * fCurrentYAxis) + tLoop->GetP1().X() * fCurrentZAxis;
                    tNextNextPoint = fCurrentOrigin + tLoop->GetP1().Y() * (cos( tNextPhi ) * fCurrentXAxis + sin( tNextPhi ) * fCurrentYAxis) + tLoop->GetP1().X() * fCurrentZAxis;
            ...
            if( KGAxialMeshRing* tRing = dynamic_cast< KGAxialMeshRing* >( *elementIt ) )
            {
                for( unsigned int tIndex = 0; tIndex < fArcCount; tIndex++ )
                {
                    ...
                    tThisThisPoint = fCurrentOrigin + tRing->GetP0().Y() * (cos( tThisPhi ) * fCurrentXAxis + sin( tThisPhi ) * fCurrentYAxis) + tRing->GetP0().X() * fCurrentZAxis;
                    tThisNextPoint = fCurrentOrigin + tRing->GetP0().Y() * (cos( tNextPhi ) * fCurrentXAxis + sin( tNextPhi ) * fCurrentYAxis) + tRing->GetP0().X() * fCurrentZAxis;
