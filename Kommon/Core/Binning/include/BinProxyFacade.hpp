...
namespace Binning{
  ...
  class BinProxyFacade : public BinInterface<DerivedIterator>{
        ...
        protected:
          auto widthImpl() const {return this->upperEdge() - this->lowerEdge();}
          auto centreImpl() const {return .5*(this->lowerEdge() + this->upperEdge());};
          void printImpl(std::ostream& output) const {output<<"["<<this->lowerEdge()<<", "<<this->upperEdge()<<"]";};
