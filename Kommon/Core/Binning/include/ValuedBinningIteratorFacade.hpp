...
namespace Binning{
  ...
  class ValuedBinningIteratorFacade : public BinningIteratorInterface<DerivedIterator> {
    ...
    protected:
      ...
      const auto& dereference(){bin = ValueType{this->derivedIterator()}; return bin;}
      void increment(){advance(1);}
      void decrement(){advance(-1);}
      void advance(difference_type n){incrementable+=n;}
      bool equal(const DerivedIterator& other) const {return incrementable == other.incrementable;}
