...
namespace Binning{
  ...
  class BinningFacade : public BinningInterface<DerivedBinning>{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::numberOfEdgesImpl() const{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::lowerEdgeImpl() const{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::upperEdgeImpl() const{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::upperEdgeImpl(std::ptrdiff_t index) const{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::BinningFacade::rangeImpl() const{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::BinningFacade::widthImpl(std::ptrdiff_t index) const{
  ...
  template <class DerivedBinning>
  auto BinningFacade<DerivedBinning>::BinningFacade::centreImpl(std::ptrdiff_t index) const{
  ...
  template <class DerivedBinning>
  bool BinningFacade<DerivedBinning>::BinningFacade::validImpl() const{

    return std::is_sorted(this->begin(), this->end(), [](const auto& bin1, const auto& bin2){return !(bin1.upperEdge() > bin2.lowerEdge());});
  ...
  bool BinningFacade<DerivedBinning>::BinningFacade::differImpl(const BinningFacade<DerivedBinning>& other) const{
    ...
    for(auto it = std::make_pair(this->begin(), other.begin()); it.first != this->end() && it.second != other.end(); ++it.first, ++it.second)
      if(it.first->centre() != it.second->centre()) return true;
  ...
  template <class DerivedBinning>
  void BinningFacade<DerivedBinning>::BinningFacade::printImpl(std::ostream& output) const{
