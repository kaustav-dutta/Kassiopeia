...
namespace Binning{
  ...
  VariableBinning::ConstIterator::ConstIterator(edge_iterator_type edgeIt)
  :ValuedBinningIteratorFacade(edgeIt){
  ...
  VariableBinning::VariableBinning(std::vector<double> edges):edges(std::move(edges)){
    ...
    if(!valid()) throw std::invalid_argument("VariableBinning: invalid edges");

  }

  VariableBinning::VariableBinning(const std::vector<ConstantBinning>& binnings){

    std::size_t numberOfEdges = std::accumulate(std::begin(binnings), std::end(binnings), 0, []
      (const auto sum, const auto& binning){return sum + binning.numberOfEdges();}
    );

    edges.reserve(numberOfEdges);

    for(const auto& binning : binnings)
      std::transform(std::begin(binning), std::end(binning), std::back_inserter(edges), [](const auto& bin){return bin.lowerEdge();});
    
    if(!binnings.empty()) edges.push_back(binnings.back().upperEdge());

    if(!valid()) throw std::invalid_argument("VariableBinning: invalid constant sub-binnings");
  ...
  std::istream& operator>>(std::istream& input, VariableBinning& binning){
    ...
    if(!constantBinnings.empty()) binning = VariableBinning{constantBinnings};
    else throw std::invalid_argument(text+" cannot be parsed to build a variable binning.");
