...
namespace Binning{
  ...
  :ValuedBinningIteratorFacade(index),binning(&binning){
  ...
  ConstantBinning::Width::Width(ConstantBinning::edge_type width):width_(width){
    ...
    if(!(width_ > 0)) throw std::invalid_argument("ConstantBinning: negative maximum bin width");
  ...
  :numberOfBins_(numberOfBins),lowerEdge_(lowerEdge),width_(numberOfBins ? (upperEdge - lowerEdge)/numberOfBins : 0){
    
    if(!valid()) throw std::invalid_argument("ConstantBinning: invalid arguments");
  ...
  ConstantBinning::ConstantBinning(Width maxBinWidth, edge_type lowerEdge, edge_type upperEdge){
    ...
    if( numberOfBins * maxBinWidth < (upperEdge - lowerEdge)) ++numberOfBins;
  ...
  void ConstantBinning::printImpl(std::ostream& output) const{
    ...
    output<<std::setw(5)<<std::left<<numberOfBins()
    <<" [ "<<std::setw(6)<<std::right<<lowerEdge()
  ...
  std::istream& operator>>(std::istream& input, ConstantBinning& binning){
    ...
    if(tokens.size() == 3)
      binning = ConstantBinning{static_cast<std::size_t>(std::stod(tokens[0])), std::stod(tokens[1]), std::stod(tokens[2])};
    else throw std::invalid_argument(text+" cannot be parsed to build a constant binning.");
