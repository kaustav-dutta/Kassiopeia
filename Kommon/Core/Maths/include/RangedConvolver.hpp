...
namespace Maths{
    ...
    public:
        ...
        double density(const SortedDistribution& distribution, double xValue) const;
    ...
    RangedConvolver<Kernel> MakeRangedConvolver(Kernel&& kernel, Args&&... args);

    template <class Kernel>
    RangedConvolver<Kernel>::RangedConvolver(Kernel&& kernel, double min, double max)
    :convolver(std::forward<Kernel>(kernel)),min(min),max(max){

        if(!(min < max)) throw std::invalid_argument("RangedConvolver: Invalid range: min >= max");
    ...
    double RangedConvolver<Kernel>::density(const SortedDistribution& distribution, double xValue) const{
        ...
        auto begin = std::find_if(std::begin(distribution), std::end(distribution), [min=this->min,xValue](const auto& point){
            return !(point.energy < xValue + min);
        });
        auto end = std::find_if(begin, std::end(distribution), [max=this->max,xValue](const auto& point){
            return !(point.energy < xValue + max);}
        );
        if(begin != std::begin(distribution)) --begin;
        if(end != std::end(distribution)) ++end;
