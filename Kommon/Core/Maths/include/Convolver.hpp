...
namespace Maths{
    ...
    public:
        ...
        double density(const Distribution& distribution, double xValue) const;
        ...
        double probability(const Distribution& distribution, const Bin& bin) const;

    };

    template <class Kernel>
    Convolver<Kernel> MakeConvolver(Kernel&& kernel);

    template <class Kernel>
    Convolver<Kernel>::Convolver(Kernel&& kernel):kernel(std::forward<Kernel>(kernel)){
