...
namespace Kommon{
    ...
    template <class Quantity, class ValueType = double>
    class PositiveValue{
    ...
    template <class Quantity, class ValueType>
    const Quantity& PositiveValue<Quantity,ValueType>::Derived() const{
    ...
    template <class Quantity, class ValueType>
    Quantity& PositiveValue<Quantity,ValueType>::Derived(){
    ...
    template <class Quantity, class ValueType>
    bool PositiveValue<Quantity,ValueType>::Invalid() const{
    ...
    template <class Quantity, class ValueType>
    void PositiveValue<Quantity,ValueType>::ThrowIfInvalid() const{

        if(Invalid()) throw KException()<<Derived().Name()<<" cannot be negative!";

    }
    template <class Quantity, class ValueType>
    PositiveValue<Quantity,ValueType>::PositiveValue():value(0.){
    ...
    template <class Quantity, class ValueType>
    PositiveValue<Quantity,ValueType>::operator ValueType() const{
