...
namespace Kommon{
  ...
  template<typename Delimiter, typename Character = char,
	   typename Traits = std::char_traits<Character>>
  class OstreamJoiner
  {
  public:
    ...
    OstreamJoiner(ostream_type& os, const Delimiter& delimiter)
    : output(os), delimiter(delimiter)
    { }

    OstreamJoiner(ostream_type& os, Delimiter&& delimiter)
    : output(os), delimiter(std::move(delimiter))
    { }
  ...
  template<typename Character, typename Traits, typename Delimiter>
    ...
    MakeOstreamJoiner(std::basic_ostream<Character, Traits>& os,
			Delimiter&& delimiter)
