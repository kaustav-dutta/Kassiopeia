    ...
    void KSGenEnergyRadonEvent::Dice( KSParticleQueue* aPrimaries )
    {
        ...
        for( tParticleIt = aPrimaries->begin(); tParticleIt != aPrimaries->end(); tParticleIt++ )
        {
            ...
            if( shakeOffElectronEnergy.size() == 0 && conversionElectronEnergy.size() == 0 )
            {
                ...
                genmsg_debug( "shell reorganization electron energies: " <<surplusEnergy << " and "<< shellElectronEnergy << eom );
            ...
            {
                genmsg_debug( "shake-off or conversion electron was produced, therefore no shell reorganization can occur" << eom );
            }
