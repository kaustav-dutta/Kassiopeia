    ...
    void KSGenPositionMask::Dice(KSParticleQueue* aPrimaries)
    {
        ...
        for(auto tParticleIt = aPrimaries->begin(); tParticleIt != aPrimaries->end(); ++tParticleIt)
        {
            ...
            while (! tPositionValid)
            {
                ...
                {
                    genmsg(eError) << "failed to find valid position after " << tNumRetries << " retries, error in generator configuration?" << eom;
                }
                ...
                if (fForbiddenSpaces.size() > 0)
                {
                    ...
                    if (! tPositionValid)
                    {
                        genmsg_debug( "diced position " << tPosition << " is inside at least one of the forbidden spaces, will retry" << eom);
                        continue;
                    }
                }
                ...
                if (fAllowedSpaces.size() > 0)
                {
                    ...
                    if (! tPositionValid)
                    {
                        genmsg_debug( "diced position " << tPosition << " is not inside any of the allowed spaces, will retry" << eom);
                        continue;
                    }
                }
            }

            genmsg( eNormal ) << "found valid position " << tPosition << " after " << tNumRetries << (tNumRetries>1?" tries":" try") << eom;
