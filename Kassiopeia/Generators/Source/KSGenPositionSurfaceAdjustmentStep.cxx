...
namespace Kassiopeia
{
    ...
    KSGenPositionSurfaceAdjustmentStep::KSGenPositionSurfaceAdjustmentStep(const KSGenPositionSurfaceAdjustmentStep& aCopy):
            KSComponent(),
    ...
    void KSGenPositionSurfaceAdjustmentStep::Dice(KSParticleQueue* aPrimaries)
    {
        for(auto tParticleIt = aPrimaries->begin(); tParticleIt != aPrimaries->end(); ++tParticleIt)
        {
            ...
            genmsg_debug("KSGenPositionSurfaceAdjustmentStep: <" << GetName() << "> set position <" << tPosition << ">" << eom);
