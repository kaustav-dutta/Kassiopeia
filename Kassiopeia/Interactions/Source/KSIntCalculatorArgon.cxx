...
namespace Kassiopeia
{
    ...
    double KSIntCalculatorArgon::GetInterpolationForTotalCrossSection( const double &anEnergy, std::map< double, double >::iterator &point ) const
    {
    ...
    double KSIntCalculatorArgon::GetUpperExtrapolationForTotalCrossSection( const double &anEnergy, std::map< double, double >::iterator & ) const
    {
        assert( anEnergy == anEnergy );
        assert( anEnergy >= 0. );
    ...
    double KSIntCalculatorArgon::GetLowerExtrapolationForTotalCrossSection( const double &, std::map< double, double >::iterator & ) const
    {
        return 0;
    }
    ...
    double KSIntCalculatorArgon::GetTheta( const double &anEnergy ) const
    {
        ...
        while( true )
        {
            ...
            if( KRandom::GetInstance().Uniform( sigma_min, sigma_max, false, true ) < GetDifferentialCrossSectionAt( anEnergy, tTheta_deg )
    ...
    void KSIntCalculatorArgonExcitation::InitializeComponent()
    {
        ...
        if( fExcitationState > 0 )
        {
            ...
            DiffCrossSectionFileName << "argon_differential_cross-section_excitation_state." << this->fExcitationState << ".txt";
    ...
    void KSIntCalculatorArgonExcitation::ExecuteInteraction( const KSParticle& anInitialParticle, KSParticle& aFinalParticle, KSParticleQueue& )
    {
        ...
        intmsg_debug( "tInitialDirection X: " << tInitialDirection.X() << " Y: " << tInitialDirection.Y() << " Z: " << tInitialDirection.Z() << ret)
        ...
        intmsg_debug( "FinalParticle: " << ret); intmsg_debug( "Time: " << aFinalParticle.GetTime() << ret); intmsg_debug( "Position: " << aFinalParticle.GetPosition().X() << " " << aFinalParticle.GetPosition().Y() << " " << aFinalParticle.GetPosition().Z() << ret ); intmsg_debug( "Momentum: " << aFinalParticle.GetMomentum().X() << " " << aFinalParticle.GetMomentum().Y() << " " << aFinalParticle.GetMomentum().Z() << ret ); intmsg_debug( "KineticEnergy: " << aFinalParticle.GetKineticEnergy_eV() << ret);
    ...
    double KSIntCalculatorArgonExcitation::GetDifferentialCrossSectionAt( const double &anEnergy, const double &anAngle ) const
    {
        assert( anEnergy == anEnergy );
        assert( anEnergy >= 0. );
        assert( anAngle == anAngle );
        ...
        if( anEnergy > maxEnergy )
        {
            return fDifferentialCrossSectionInterpolator->GetValue( maxEnergy - 1., anAngle ) / this->GetTotalCrossSectionAt( maxEnergy - 1. ) * (this->GetTotalCrossSectionAt( anEnergy ));
        }
        else
        {
            return fDifferentialCrossSectionInterpolator->GetValue( anEnergy, anAngle );
    ...
    void KSIntCalculatorArgonElastic::ExecuteInteraction( const KSParticle& anInitialParticle, KSParticle& aFinalParticle, KSParticleQueue& )
    {
    ...
    void KSIntCalculatorArgonSingleIonisation::ExecuteInteraction( const KSParticle& anInitialParticle, KSParticle& aFinalParticle, KSParticleQueue& aSecondaries )
    {
        double tInitialKineticEnergy = anInitialParticle.GetKineticEnergy_eV();
        KThreeVector tInitialDirection = anInitialParticle.GetMomentum();

        intmsg_debug( "< " << this->GetName() << " >:ExecuteInteraction() " << ret ); intmsg_debug( "tInitialKineticEnergy: " << tInitialKineticEnergy << ret);
        intmsg_debug( "tInitialDirection X: " << tInitialDirection.X() << " Y: " << tInitialDirection.Y() << " Z: " << tInitialDirection.Z() << ret)
        ...
        intmsg_debug( "SecondaryParticle: " << ret); intmsg_debug( "Time: " << tSecondary->GetTime() << ret); intmsg_debug( "Position: " << tSecondary->GetPosition().X() << " " << tSecondary->GetPosition().Y() << " " << tSecondary->GetPosition().Z() << ret ); intmsg_debug( "Momentum: " << tSecondary->GetMomentum().X() << " " << tSecondary->GetMomentum().Y() << " " << tSecondary->GetMomentum().Z() << ret ); intmsg_debug( "KineticEnergy: " << tSecondary->GetKineticEnergy_eV() << ret);
        intmsg_debug( "=============<" << this->GetName() << ">:ExecuteInteraction()" << eom)
    ...
    void KSIntCalculatorArgonDoubleIonisation::ExecuteInteraction( const KSParticle& anInitialParticle, KSParticle& aFinalParticle, KSParticleQueue& aSecondaries )
    {
        double tInitialKineticEnergy = anInitialParticle.GetKineticEnergy_eV();
        KThreeVector tInitialDirection = anInitialParticle.GetMomentum();
        assert( tInitialKineticEnergy == tInitialKineticEnergy );
...
double KSIntCalculatorArgonDoubleIonisation::GetTheta( const double &anEnergy, const double &anEloss ) const
{
    ...
    while( true )
    {
        ...
        if( KRandom::GetInstance().Uniform( sigma_min, sigma_max, false, true ) < GetDifferentialCrossSectionAt( anEnergy, tTheta_deg, anEloss ) )
            break;
    }
    assert( tTheta_deg == tTheta_deg );
    assert( tTheta_deg >= 0. && tTheta_deg <= 180. );
...
KSIntCalculatorArgonTotalCrossSectionReader::KSIntCalculatorArgonTotalCrossSectionReader( std::istream *aStream, unsigned int numOfParameters )
{
    fStream = aStream;
...
KSIntCalculatorArgonDifferentialCrossSectionReader::KSIntCalculatorArgonDifferentialCrossSectionReader( std::istream *aStream, unsigned int numOfParameters )
{
    fStream = aStream;
    fNumOfParameters = numOfParameters;
    fData = new std::map< double*, double >();
