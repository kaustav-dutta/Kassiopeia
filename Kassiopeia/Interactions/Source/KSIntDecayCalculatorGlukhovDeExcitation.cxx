...
namespace Kassiopeia
{
    ...
    KSIntDecayCalculatorGlukhovDeExcitation::KSIntDecayCalculatorGlukhovDeExcitation( const KSIntDecayCalculatorGlukhovDeExcitation& aCopy ) :
            KSComponent(),
    ...
    const double KSIntDecayCalculatorGlukhovDeExcitation::b_dex[3][3][3] = { {{17.287,1.1094,-0.4820},
                                                                              {-24.886,13.505,-5.6676},
                                                                              {11.780,-10.330,4.1794}},

                                                                             {{1.8683, 0.1071, -0.0463},
                                                                              {-2.7344, 1.5025, -0.6243},
                                                                              {1.2669, -1.1082, 0.4502}},

                                                                             {{5.3091,  0.3091,  -0.1331},
                                                                              {-6.2159,  1.3430, -0.2841},
                                                                              {3.0276,  -2.0534,  0.7185 }} };

    void KSIntDecayCalculatorGlukhovDeExcitation::CalculateLifeTime( const KSParticle& aParticle,
                                                                      double& aLifeTime )
    {
        ...
        if ( (tPID == fTargetPID && fTargetPID != 0) || ( (tPID >= fminPID) && ( tPID <= fmaxPID)) )
        {
            ...
            aLifeTime = 1./( CalculateSpontaneousDecayRate(n,1)
                             *CalculateRelativeDeExcitationRate(n,1,fTemperature));
        } else
        {
    ...
    {
        if (l > 2 || l < 0 || n<0)
            intmsg(eError) << "KSIntDecayCalculatorGlukhovDeExcitation: Only l states 0,1,2 available for Rydberg decay" << eom;
