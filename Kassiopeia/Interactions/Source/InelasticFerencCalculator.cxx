...
namespace Kassiopeia
{
    ...
    double InelasticFerencCalculator::sigmaexc( double anE )
    {
        ...
        double sigma;
        if( anE < 11.18 ) //no excitation below this energy, otherwise your primary can have negative energy after the process
            sigma = 0.0;
    ...
    void InelasticFerencCalculator::randomexc( double anE, double& Eloss, double& Theta ) //Todo: move static stuff to constructor. constants and precision?
    {
        ...
        double En[ 7 ] =
        { 12.73 / 27.2, 13.2 / 27.2, 14.77 / 27.2, 15.3 / 27.2, 14.93 / 27.2, 15.4 / 27.2, 13.06 / 27.2 };
        ...
        double EB[ 28 ] =
        { 0.411, 0.417, 0.423, 0.428, 0.434, 0.439, 0.444, 0.449, 0.454, 0.459, 0.464, 0.468, 0.473, 0.477, 0.481, 0.485, 0.489, 0.493, 0.496, 0.500, 0.503, 0.507, 0.510, 0.513, 0.516, 0.519, 0.521, 0.524 };
        ...
        double pB[ 28 ] =
        { 4.2e-3, 1.5e-2, 3.0e-2, 4.7e-2, 6.3e-2, 7.3e-2, 7.9e-2, 8.0e-2, 7.8e-2, 7.3e-2, 6.6e-2, 5.8e-2, 5.1e-2, 4.4e-2, 3.7e-2, 3.1e-2, 2.6e-2, 2.2e-2, 1.8e-2, 1.5e-2, 1.3e-2, 1.1e-2, 8.9e-3, 7.4e-3, 6.2e-3, 5.2e-3, 4.3e-3, 3.6e-3 };
...
//old version (HYDROGEN ONLY!)
        ...
        if( anE > fOrbitalEnergy[ fMinimum ] )
        {
            ...
            for( unsigned int i = 0; i < CrossSections.size(); i++ )
            {
                ...
                if( IonizationDice < 0 )
                {
                    ...
                    intmsg_debug( "InelasticFerencCalculator::sigmaion"<<ret ); intmsg_debug("ionization energy: " << CrossSections.at(i) << eom );
    ...
    bool InelasticFerencCalculator::ReadData()
    {
        ...
        if( fDataFile->Open( KFile::eRead ) == true )
        {
            ...
            while( !inputfile.eof() )
            {
                ...
                if( c >= '0' && c < '9' )
                {
                    ...
                    intmsg_debug( "InelasticFerencCalculator::ReadData "<<ret ); intmsg_debug( aTemp << " " << anotherTemp << " " << aTempInt << eom );
                ...
                else
                {
                    ...
                    intmsg_debug( "InelasticFerencCalculator::ReadData "<<ret ); intmsg_debug( "dumping " << dump << " because " << c <<" is not a number" << eom );
                    continue;
                }
            }
        }
        else
        {
            ...
            intmsg( eError ) << "InelasticFerencCalculator::ReadData: FATAL ERROR reading scattering data: inputfile <" << fMoleculeType << ".txt> not found or molecule type not supported. " << eom;
        }
        fDataFile->Close();
...
//private helper methods
    ...
    {
        double Kvec[ 15 ] =
        { 0., 0.1, 0.2, 0.4, 0.6, 0.8, 1., 1.2, 1.5, 1.8, 2., 2.5, 3., 4., 5. };
        double fvec[ 7 ][ 15 ] =
        {
        ...
        double EeV[ 7 ] =
        { 12.73, 13.20, 14.77, 15.3, 14.93, 15.4, 13.06 };
        int jmin = 0;
        int nnmax = 6;
    ...
    double InelasticFerencCalculator::sigmadiss10( double anE )
    {
        ...
        double a[ 9 ] =
        { -2.297914361e5, 5.303988579e5, -5.316636672e5, 3.022690779e5, -1.066224144e5, 2.389841369e4, -3.324526406e3, 2.624761592e2, -9.006246604 };
    ...
    double InelasticFerencCalculator::sigmadiss15( double anE )
    {
        ...
        double a[ 9 ] =
        { -1.157041752e3, 1.501936271e3, -8.6119387e2, 2.754926257e2, -5.380465012e1, 6.573972423, -4.912318139e-1, 2.054926773e-2, -3.689035889e-4 };
        double lnsigma, lnE, lnEn, Emin, sigma;
        int n;
        //  anE is in eV
        sigma = 0.;
