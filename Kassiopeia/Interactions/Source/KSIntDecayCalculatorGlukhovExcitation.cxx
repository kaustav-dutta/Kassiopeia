...
namespace Kassiopeia
{
    ...
    KSIntDecayCalculatorGlukhovExcitation::KSIntDecayCalculatorGlukhovExcitation( const KSIntDecayCalculatorGlukhovExcitation& aCopy ) :
            KSComponent(),
    ...
    const double KSIntDecayCalculatorGlukhovExcitation::b_ex[3][3][3] = {{{14.557 , 0.7418 , -0.0437},
                                                                         {-8.8432 , 5.3981 , -2.6255},
                                                                         {10.9688 , -19.720 , 9.0861}},

                                                                        {{1.5675 , 0.0791 , -0.0045},
                                                                         {-1.0648 , 0.7484 , -0.3610},
                                                                         {0.9096 , -1.5654 , 0.6850}},

                                                                        {{4.4430 , 0.2415 ,-0.0212},
                                                                        {-2.4783 , 1.2168 ,-0.5731},
                                                                        { 2.9479 ,-5.1645 , 2.2992}}};


    void KSIntDecayCalculatorGlukhovExcitation::CalculateLifeTime( const KSParticle& aParticle,
                                                                      double& aLifeTime )
    {
        ...
        if ( (tPID == fTargetPID && fTargetPID != 0) || ( (tPID >= fminPID) && ( tPID <= fmaxPID)) )
        {
            ...
            aLifeTime = 1./( CalculateSpontaneousDecayRate(n,1)
                             *CalculateRelativeExcitationRate(n,1,fTemperature));
        } else
        {
    ...
    {
        if (l > 2 || l < 0 || n<0)
            intmsg(eError) << "KSIntDecayCalculatorGlukhovExcitation: Only l states 0,1,2 available for Rydberg decay" << eom;
