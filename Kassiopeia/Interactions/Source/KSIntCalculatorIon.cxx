...
namespace Kassiopeia
{
								      ...
								      )
  {
    ...
    if (fGas.compare("H2O") == 0) {
        shell_list = {{12.61,2},{14.73,2},{18.55,2},{32.2,2},{539.7,2}};  //list of I and N from Table 1b in Rudd 1992
    }
  ...
  double KSIntCalculatorIon::w_c(double v,double I) {
    ...
    void KSIntCalculatorIon::CalculateSecondaryElectronAngle(double& aSecondaryElectronAngle)
    {
        ...
        double sigma_max;
        CalculateAngleDifferentialCrossSection( aSecondaryElectronMaxAngle,
                                                sigma_max
                                                );
        ...
        double sigma_min;
        CalculateAngleDifferentialCrossSection( aSecondaryElectronMinAngle,
                                                sigma_min
                                                );
        ...
        while ( true )
        {
            ...
            tSecondaryElectronAngle = KRandom::GetInstance().Uniform( aSecondaryElectronMinAngle,
                                                                      aSecondaryElectronMaxAngle,
                                                                      false, true
                                                                      );
            //Get the diff. cross section for this electron angle
            CalculateAngleDifferentialCrossSection( tSecondaryElectronAngle,
                                                    tCrossSection
                                                    );
            //Randomly select a diff. cross section
            double tRandom = KRandom::GetInstance().Uniform( 0.,
