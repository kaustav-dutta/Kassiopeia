...
namespace Kassiopeia {
    ...
    FBBRionization::FBBRionization(double aT, int an, int al):
            fT( aT ),
            fn( an ),
            fl( al)
    {
    ...
    double FBBRionization::RadInt2BoundFreeBurgess(int n, int l, double E, int sign) {
        ...
        double logg1=0.5*(logPs-log(1.-exp(-2.*M_PI/k)))+2*n-2./k*atan(n*k)-(n+2)*log(D)+logg0;
...
//              printf("L,g,Exponent= %12i  %12.2e  %12.3f  \t\n",L,g,Exponent);
    ...
    double RydbergCalculator::HypergeometricFHoangBinh(int a, int b, int c, double x, double &E) {
        if (b > 0 || c <= 0) {
            intmsg(eError) << "Message from function HypergeometricFHoangBinh:" << ret
            << "b>0 or c<=0!" << ret
            << "b,c= " << b << ", " << c << ret
    ...
    double RydbergCalculator::RadInt2Gordon(int n, int l, int np, int sign) {
        ...
        if (n < 1 || np < 1 || l < 0 || l > n - 1) {
            intmsg(eError)
            << "Message from function RadInt2Gordon:" << ret
            << "n and np have to be larger than 0!" << ret
            << "l has to be larger than -1 and smaller than n !" << ret
            << "Program running is stopped !!! " << eom;
        }
        ...
        if (n > nMAX || np > nMAX) {
            intmsg(eError)
            << "Message from function RadInt2Gordon" << ret
            << "n and np cannot be larger than nMAX!" << ret
            << "Program running is stopped !!! " << eom;
        }
        ...
        if (sign != -1 && sign != +1) {
            intmsg(eError)
            << "Message from function RadInt2Gordon:" << ret
            << "sign has to be either -1 or +1!" << ret
            << "Program running is stopped !!! " << eom;
        }
