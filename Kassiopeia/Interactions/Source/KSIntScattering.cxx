...
//        fCalculators.clear();
    ...
    void KSIntScattering::CalculateAverageCrossSection(
    ...
    void KSIntScattering::CalculateInteraction(
    ...
    void KSIntScattering::SetDensity( KSIntDensity* aDensityCalculator )
    {
        ...
        intmsg( eError ) << "cannot set density calculator <" << aDensityCalculator->GetName() << "> to scattering module <" << GetName() << ">" << eom;
        return;
    }
    ...
    void KSIntScattering::AddCalculator( KSIntCalculator* aScatteringCalculator )
    {
        ...
        for( tIt = fCalculators.begin(); tIt != fCalculators.end(); tIt++ )
        {
            ...
            {
                intmsg( eError ) << "could not add scattering calculator <" << aScatteringCalculator->GetName() << "> to scattering module <" << GetName() << ">" << eom;
                return;
            }
        }
        fCalculators.push_back( aScatteringCalculator );
        fCrossSections.resize( fCalculators.size(), 0. );
        intmsg_debug( "added scattering calculator <" << aScatteringCalculator->GetName() << "> to scattering module <" << GetName() << ">" << eom );
        return;
    }
    void KSIntScattering::RemoveCalculator( KSIntCalculator* aScatteringCalculator )
    {
        ...
        for( tIt = fCalculators.begin(); tIt != fCalculators.end(); tIt++ )
        {
            ...
            if( tCalculator == aScatteringCalculator )
            {
                ...
                fCrossSections.resize( fCalculators.size(), 0. );
                intmsg_debug( "removed scattering calculator <" << aScatteringCalculator->GetName() << "> to scattering module <" << GetName() << ">" << eom );
                return;
            }
        }
        intmsg( eError ) << "could not remove scattering calculator <" << aScatteringCalculator->GetName() << "> to scattering module <" << GetName() << ">" << eom;
        return;
    }
