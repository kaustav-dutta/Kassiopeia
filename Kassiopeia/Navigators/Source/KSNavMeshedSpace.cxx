...
namespace Kassiopeia
{
    ...
    void
    KSNavMeshedSpace::CollectMeshElements()
    {
        //temp space
        KSSpace* tSpace;
        fSpaces.clear();
    ...
    void
    KSNavMeshedSpace::ConstructTree()
    {
        ...
        {
            navmsg(eNormal) << "navigation space <" << this->GetName() << "> reconstructed an octree from <"<<fFileName<<">" << eom;
        }
    ...
    void KSNavMeshedSpace::SaveTree()
    {
        ...
        std::stringstream se;
        for(unsigned int i=0; i<fSpaces.size(); i++){se << fSpaces[i]->GetName();}
        for(unsigned int i=0; i<fSides.size(); i++){se << fSides[i]->GetName();}
        for(unsigned int i=0; i<fSurfaces.size(); i++){se << fSurfaces[i]->GetName();}
        se << fElementMap.size();
        ...
        if(fSpecifyMaxDepth){ sp << fMaxDepth; };
        if(fSpecifyResolution){ sp << fSpecifyResolution; };
        if(fSpecifyAllowedElements){ sp << fNAllowedElements; };
        temp = sp.str();
        ...
        if(fSpecifyMaxDepth){ data.SetSpecifyMaximumOctreeDepth(1);};

        data.SetSpatialResolution(fSpatialResolution);
        data.SetSpecifySpatialResolution(0);
        if(fSpecifyResolution){data.SetSpecifySpatialResolution(1);};

        data.SetNumberOfAllowedElements(fNAllowedElements);
        data.SetSpecifyNumberOfAllowedElements(0);
        if(fSpecifyAllowedElements){data.SetSpecifyNumberOfAllowedElements(1);};
    ...
    bool KSNavMeshedSpace::RetrieveTree()
    {
        ...
        else
        {
            ...
            for(unsigned int i=0; i<KGMESH_DIM; i++){ dim[i] = 2;}; //we use an octtree in 3d
            ...
            for(unsigned int i=0; i<n_nodes; i++)
            {
                ...
                KGObjectRetriever<KGMeshNavigationNodeObjects, KGSpaceTreeProperties<KGMESH_DIM> >::SetNodeObject(tree_prop, tree_nodes[i]);

                //attach the mesh element container
                KGObjectRetriever<KGMeshNavigationNodeObjects, KGNavigableMeshElementContainer >::SetNodeObject(&fElementContainer, tree_nodes[i]);
            }
    ...
    void KSNavMeshedSpace::CalculateNavigation( const KSTrajectory& aTrajectory, const KSParticle& aTrajectoryInitialParticle,
        const KSParticle& aTrajectoryFinalParticle, const KThreeVector& aTrajectoryCenter, const double& aTrajectoryRadius,
        const double& aTrajectoryStep, KSParticle& aNavigationParticle, double& aNavigationStep, bool& aNavigationFlag )
    {
        ...
        if(fFailCheck)
        {
            ...
            {
                navmsg( eWarning ) <<"inital position <" << tInitialPoint.X() <<", "<< tInitialPoint.Y() <<", "<< tInitialPoint.Z() <<"> is outside of octree search volume"<<eom;
        ...
        if( fProximityChecker.SphereIntersectsMesh() )
        {
            ...
            aTrajectory.GetPiecewiseLinearApproximation(aTrajectoryInitialParticle, aTrajectoryFinalParticle, &fIntermediateParticleStates);
            ...
            while( stopIt != fIntermediateParticleStates.end() )
            {
                ...
                if( fFirstIntersectionFinder.HasIntersectionWithMesh() )
                {
                    ...
                    navmsg_debug( "navigation space <" << this->GetName() << "> has detected a possible intersection" << eom );
                    ...
                    //sanity check
                    if( tTime > stopIt->GetTime() ){tTime = stopIt->GetTime();};
                    if( tTime < startIt->GetTime() ){tTime = startIt->GetTime();};
                    ...
                    switch (fElementMap[fMeshElementID].fType)
                    {
                        case KSNAVMESHEDSPACE_SPACE:
                            ...
                            {
                                if(dist_from_last > fRelativeTolerance*length){dist_to_add = fAbsoluteTolerance;}
                                ...
                                else if(fLastSpaceEntity == fSpaceEntity )
                                {
                                    ...
                                    navmsg_debug( "  invalid intersection detected near previous intersection on "<< fSpaceEntity->GetName() << ">" << eom );
                                    validIntersection = false;
                                }
                            }
                            else if(fLastSpaceEntity == fSpaceEntity  || fLastSpaceEntity == nullptr)
                            {
                                ...
                                if( (fNavigationCount - fLastSpaceCount <= 1) && (dist_from_last < length) )
                                {
                                    ...
                                    navmsg(eWarning) <<  " skipping segment because invalid intersection detected near previous intersection at on <" << fSpaceEntity->GetName() << ">, your navigator tolerances are probably too small!" << eom;
                        ...
                        case KSNAVMESHEDSPACE_SIDE:
                            ...
                            {
                                if(dist_from_last > fRelativeTolerance*aTrajectoryRadius){dist_to_add = fAbsoluteTolerance;}
                                ...
                                {
                                    //same entity or very close to last intersection
                                    navmsg_debug( "  invalid intersection detected near previous intersection at on <" << fSideEntity->GetName() << ">" << eom );
                                    validIntersection = false;
                                }
                            }
                            else if(fLastSideEntity == fSideEntity  || fLastSideEntity == nullptr)
                            {
                                ...
                                if( (fNavigationCount - fLastSideCount <= 1) && (dist_from_last < length) )
                                {
                                    ...
                                    //but because the absolute/relative tolerances are too tight
                                    //it has not been detected, so we issue a debug warning and skip this segment
                                    navmsg(eWarning) << " skipping segment because invalid intersection detected near previous intersection at on <" << fSideEntity->GetName() << ">, your navigator tolerances are probably too small!" << eom ;
                                    validIntersection = false;
                                    skipSegment = true;
                                }
                            }
                        ...
                        case KSNAVMESHEDSPACE_SURFACE:
                            ...
                            if(dist_from_last < fRelativeTolerance*aTrajectoryRadius || dist_from_last < fAbsoluteTolerance)
                            {
                                ...
                                {
                                    //same entity or very close to last intersection
                                    navmsg_debug( "  invalid intersection detected near previous intersection at on <" << fSurfaceEntity->GetName() << ">" << eom );
                                    validIntersection = false;
                                }
                            }
                            else if(fLastSurfaceEntity == fSurfaceEntity  || fLastSurfaceEntity == nullptr)
                            {
                                ...
                                if( (fNavigationCount - fLastSurfaceCount <= 1) && (dist_from_last < length) )
                                {
                                    ...
                                    //but because the absolute/relative tolerances are too tight
                                    //it has not been detected, so we issue a debug warning and skip this segment
                                    navmsg(eWarning) <<  " skipping segment because invalid intersection detected near previous intersection at on <" << fSurfaceEntity->GetName() << ">, your navigator tolerances are probably too small!" << eom;
                                    validIntersection = false;
                                    skipSegment = true;
                                }
                            }
                    ...
                    if(validIntersection) //intersection is not a repeat of the last
                    {
                        ...
                        navmsg_debug( "navigation space <" << this->GetName() << "> detected valid intersection after time step: "<<aNavigationStep  << eom );
                    ...
                    else
                    {
                        ...
                        if(repeatCount < 4) //avoid infinite loop in case of severe problems
                        {
                            ...
                            if(distance < length)
                            {
                                navmsg_debug( " incrementing intersection search by <" << dist_to_add << "> and continuing " << eom );
                            ...
                            {
                                navmsg_debug( "  skipping trajectory segment because it is entirely within tolerance of the last intersection " << eom );
                                //this entire segment is too close to last intersection
                                //go to next one
                                startIt++;
                                stopIt++;
    ...
    void KSNavMeshedSpace::ExecuteNavigation( const KSParticle& aNavigationParticle, KSParticle& aFinalParticle, KSParticleQueue& aParticleQueue ) const
    {
    ...
    double
    KSNavMeshedSpace::SolveForTime(double distance, double t, double v1, double v2) const
    {
        ...
        if(x1 < 0.0 || x1 > t){x1_is_valid = false;};
        if(x2 < 0.0 || x1 > t){x2_is_valid = false;};
        ...
        else
        {
            ...
            if(x1_is_valid){return x1;};
            if(x2_is_valid){return x2;};
        }
