...
namespace Kassiopeia
{
    ...
    */
    ...
    class KSNavMeshedSpace :
        public KSComponentTemplate< KSNavMeshedSpace, KSSpaceNavigator >
    {
        public:
            KSNavMeshedSpace();
            ...
            void SetEnterSplit( const bool& anEnterSplit ){fEnterSplit = anEnterSplit;};
            bool GetEnterSplit() const {return fEnterSplit; };

            void SetExitSplit( const bool& anExitSplit ){fExitSplit = anExitSplit;};
            bool GetExitSplit() const {return fExitSplit; };

            void SetFailCheck( const bool& aValue ){fFailCheck = aValue;};
            bool GetFailCheck() const {return fFailCheck;};

            void SetRootSpace(KSSpace* root_space){fRootSpace = root_space;};
            KSSpace* GetRootSpace() const {return fRootSpace;};

            void SetMaximumOctreeDepth(unsigned int d){fMaxDepth = d; fSpecifyMaxDepth = true;};
            unsigned int GetMaximumOctreeDepth() const {return fMaxDepth;};

            void SetSpatialResolution(double r){fSpatialResolution = r; fSpecifyResolution = true;};
            ...
            void SetAbsoluteTolerance(double abs_tol){fAbsoluteTolerance = abs_tol; fUserSpecifiedAbsoluteTolerance = true;};
            double GetAbsoluteTolerance() const {return fAbsoluteTolerance;};

            void SetRelativeTolerance(double rel_tol){fRelativeTolerance = rel_tol;};
            double GetRelativeTolerance() const {return fRelativeTolerance;};
...
//            MeshElementAssociationSortingPredicate fSortingPred;
            ...
            class KSMeshElementCollector:
                public KGMeshElementCollector
            {
                public:
                    ...
                    void PreCollectionActionExecute(KGMeshData* aData ) override
                    {
                        ...
                        if(n_elem == 1)
                        {
                            ...
                            {
                                navmsg( eWarning ) << " space <" << fSpace->GetName() << "> has only 1 mesh element, please increase the discretization parameters for this space if you wish to use the meshed space navigator." << eom;
                                if(sum_area == 0.0)
                                {
                                    navmsg( eWarning ) << " navigator will ignore the mesh elements of space <" << fSpace->GetName() << "> which have a cumulative area of zero!" << eom;
                                }
                            }

                            if(fType == KSNAVMESHEDSPACE_SIDE)
                            {
                                navmsg( eWarning ) << " side <" << fSide->GetName() << "> has only 1 mesh element, please increase the discretization parameters for this side if you wish to use the meshed space navigator." << eom;
                                if(sum_area == 0.0)
                                {
                                    navmsg( eWarning ) << " navigator will ignore the mesh elements of side <" << fSide->GetName() << "> which have a cumulative area of zero!" << eom;
                                }
                            }

                            if(fType == KSNAVMESHEDSPACE_SURFACE)
                            {
                                navmsg( eWarning ) << " surface <" << fSurface->GetName() << "> has only 1 mesh element, please increase the discretization parameters for this surface if you wish to use the meshed space navigator." << eom;
                                if(sum_area == 0.0)
                                {
                                    navmsg( eWarning ) << " navigator will ignore the mesh elements of surface <" << fSurface->GetName() << "> which have a cumulative area of zero!" << eom;
                                }
                            }
                        }
