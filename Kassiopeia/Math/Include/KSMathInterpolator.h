...
namespace Kassiopeia
{
    ...
    class KSMathInterpolator< KSMathSystem< XValueType, XDerivativeType, XErrorType > >
    {
        ...
        public:
            virtual void Interpolate( double aTime,
    ...
    template< class XValueType, class XDerivativeType, class XErrorType >
    double
    KSMathInterpolator< KSMathSystem< XValueType, XDerivativeType, XErrorType > >::DistanceMetric
    (
        const XValueType& valueA,
        const XValueType& valueB
    )
    const
    {
    ...
    (
        ...
        const XValueType& anInitialValue,
        const XValueType& aFinalValue,
        std::vector<XValueType>* interpolatedValues
    )
    const
    {
        ...
        {
            tFirst = tValues.begin(); tNext = tFirst; tNext++;
            tFirstTime = tValueTimes.begin(); tNextTime = tFirstTime; tNextTime++;
            ...
            while( (tNext != tValues.end()) && (segmentCount < nMaxSegments) )
            {
                ...
                if(tFirst->second)
                {
                    ...
                    Interpolate(anInitialTime, anIntegrator, aDifferentiator, anInitialValue, aFinalValue, tTimeStep, tCurveMidPoint);
                    ...
                    if(dist > aTolerance)
                    {
                        ...
                        tFirstTime++;
                        tNextTime = tFirstTime; tNextTime++;
