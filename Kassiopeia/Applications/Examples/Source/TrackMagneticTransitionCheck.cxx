...
int main()
{
    ...
    int cell_count = 0;
    double Bxx_correlation_total = 0., Byy_correlation_total = 0., Bzz_correlation_total = 0., Bxz_correlation_total = 0.;
    ...
    for( ; tRunReader <= tRunReader.GetLastRunIndex(); tRunReader++ )
    {
  		for( ; tEventReader <= tRunReader.GetLastEventIndex(); tEventReader++ )
  		{
              ...
              B_list.push_back( KThreeVector( 1., std::cos( frequency_gap * tTime.Value() ), std::sin( frequency_gap * tTime.Value() ) ) );
    ...
    double TransitionRate = 1. / 16. * gyromagnetic_ratio * gyromagnetic_ratio * ( std::cos( theta ) * std::cos( theta ) * Bxx_correlation  + Byy_correlation
    ...
    std::cout << "Total correlations: " << Bxx_correlation_total << "  " << Byy_correlation_total << "  " << Bzz_correlation_total << "  " << Bxz_correlation_total << "\n";
