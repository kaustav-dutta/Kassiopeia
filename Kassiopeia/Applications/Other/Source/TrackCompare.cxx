...
const int graphcolors[14] = { kBlack, kRed+2, kGreen+2, kBlue+2, kMagenta+2, kYellow+2, kCyan+2, kPink-1, kSpring-1, kAzure-1, kOrange-1, kTeal-1, kViolet-1, kGray };

#define READ_VALUE( xREADER, xNAME, xDEFAULT, xTYPE, xGETTER )          \
                ( xREADER.Exists< xTYPE >( xNAME )                      \
                ? xREADER.Get< xTYPE >( xNAME ).xGETTER                 \
                : (xDEFAULT) )
#define READ_STRING( xREADER, xNAME, xDEFAULT )                         \
                READ_VALUE( xREADER, xNAME, xDEFAULT, KSString, Value() )
#define READ_INT( xREADER, xNAME, xDEFAULT )                            \
                READ_VALUE( xREADER, xNAME, xDEFAULT, KSInt, Value() )
#define READ_UINT( xREADER, xNAME, xDEFAULT )                           \
                READ_VALUE( xREADER, xNAME, xDEFAULT, KSUInt, Value() )
#define READ_DOUBLE( xREADER, xNAME, xDEFAULT )                         \
                READ_VALUE( xREADER, xNAME, xDEFAULT, KSDouble, Value() )
#define READ_VECTOR_Z( xREADER, xNAME, xDEFAULT )                       \
                READ_VALUE( xREADER, xNAME, xDEFAULT, KSThreeVector, Value().Z() )
#define READ_VECTOR_R( xREADER, xNAME, xDEFAULT )                       \
                READ_VALUE( xREADER, xNAME, xDEFAULT, KSThreeVector, Value().Perp() )
...
#define UPDATE_GRAPH( xNAME, xXVALUE, xYVALUE )                         \
                sGraphs.xNAME->SetPoint( sGraphs.xNAME->GetN(), (xXVALUE), (xYVALUE) );
...
#define PRINT_GRAPH( xNAME )                                            \
                std::cout << "  " << #xNAME << ":"                      \
                    ...
                    << "\tmean=" << sGraphs.xNAME->GetMean()            \
                    << "\trms="  << sGraphs.xNAME->GetRMS( 2 )          \
                    << "\tarea=" << sGraphs.xNAME->Integral()           \
                    << std::endl;
                ...
                if (tStepReader.HasObject( "output_step_world" ))
                {
                    ...
                    for( tStepReader = tTrackReader.GetFirstStepIndex(); tStepReader <= tTrackReader.GetLastStepIndex(); tStepReader++ )
                    {
                        ...
                        if ( ! tIsFirstStep )
                        {
                            ...
                            double tMagneticMomentChange    = 2. * (tMagneticMoment - tLastMagneticMoment) / (tMagneticMoment + tLastMagneticMoment);
                            double tTotalEnergyChange       = 2. * (tTotalEnergy - tLastTotalEnergy) / (tTotalEnergy + tLastTotalEnergy);
...
int main(int argc, char **argv)
{
    ...
    opt.add_options()
        ...
        ( "run,R",          po::value< vector<unsigned int> >(),                "set run # to process" )
        ( "event,E",        po::value< vector<unsigned int> >(),                "set event # to process" )
        ( "track,T",        po::value< vector<unsigned int> >(),                "set track # to process" )
        ( "terminator,t",   po::value< string >(),                              "select tracks by terminator" )
        ( "generator,g",    po::value< string >(),                              "select tracks by generator" )
        ( "nomirror,M",                                                         "do not show reflected electrons" )
        ( "file,f",         po::value< vector<string> >(),                      "ROOT input file" )
        ( "output,o",       po::value< string >()->default_value( argv[0] ),    "output filename (without extension)" )
    ;
    ...
    tCanvas->cd( 2 );   DRAW_MULTIGRAPH( Radius,                "AL", "s [m]", "r [m]" );
    tCanvas->cd( 3 );   DRAW_MULTIGRAPH( Steps,                 "AL", "s [m]", "n [1]" );

    tCanvas->cd( 4 );   DRAW_MULTIGRAPH( ElectricPotential,     "AL", "s [m]", "U [V]" );
    tCanvas->cd( 5 );   DRAW_MULTIGRAPH( ElectricField,         "AL", "s [m]", "|E| [V/m]" );
    tCanvas->cd( 6 );   DRAW_MULTIGRAPH( MagneticField,         "AL", "s [m]", "|B| [T]" );

    tCanvas->cd( 7 );   DRAW_MULTIGRAPH( Time,                  "AL", "s [m]", "t [s]" );
    tCanvas->cd( 8 );   DRAW_MULTIGRAPH( Speed,                 "AL", "s [m]", "v [m/s]" );
    tCanvas->cd( 9 );   DRAW_MULTIGRAPH( Acceleration,          "AL", "s [m]", "a [m/s^2]" );

    tCanvas->cd( 10 );  DRAW_MULTIGRAPH( KineticEnergy,         "AL", "s [m]", "E_{kin} [eV]" );
    tCanvas->cd( 11 );  DRAW_MULTIGRAPH( PolarAngle,            "AL", "s [m]", "#theta [deg]" );
    tCanvas->cd( 12 );  DRAW_MULTIGRAPH( AziAngle,              "AL", "s [m]", "#phi [deg]" );

    tCanvas->cd( 13 );  DRAW_MULTIGRAPH( LongKineticEnergy,     "AL", "s [m]", "E_{long} [eV]" );
    tCanvas->cd( 14 );  DRAW_MULTIGRAPH( PolarAngleChange,      "AL", "s [m]", "#Delta #theta [deg]" );
    tCanvas->cd( 15 );  DRAW_MULTIGRAPH( StepLength,            "AL", "s [m]", "#Delta s [m]" );

    tCanvas->cd( 16 );  DRAW_MULTIGRAPH( MagneticMomentChange,  "AL", "s [m]", "#delta #mu [1]" );
    tCanvas->cd( 17 );  DRAW_MULTIGRAPH( TotalEnergyChange,     "AL", "s [m]", "#delta E [1]" );
    tCanvas->cd( 18 );  DRAW_MULTIGRAPH( CumulTotalEnergyChange,"AL", "s [m]", "#Sigma(#Delta E) [eV]" );
