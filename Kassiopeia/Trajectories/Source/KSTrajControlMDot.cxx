...
namespace Kassiopeia
{
    ...
    KSTrajControlMDot::KSTrajControlMDot( const KSTrajControlMDot& aCopy ) :
            KSComponent(),
    ...
    void KSTrajControlMDot::Calculate( const KSTrajAdiabaticSpinParticle& aParticle, double& aValue )
    {
      ...
      double tMDot = std::sqrt( 1 - aParticle.GetAlignedSpin() * aParticle.GetAlignedSpin() ) * aParticle.GetVelocity().Dot( GradBDirection * c );
      double tPhiDot = aParticle.GetGyromagneticRatio() * aParticle.GetMagneticField().Magnitude() - aParticle.GetVelocity().Dot( A ) - aParticle.GetAlignedSpin() / std::sqrt( 1 - aParticle.GetAlignedSpin() * aParticle.GetAlignedSpin() ) * aParticle.GetVelocity().Dot( GradBDirection * a );
    ...
    void KSTrajControlMDot::Check( const KSTrajAdiabaticSpinParticle&, const KSTrajAdiabaticSpinParticle&, const KSTrajAdiabaticSpinError&, bool& aFlag )
    {
        aFlag = true;
        return;
    }
