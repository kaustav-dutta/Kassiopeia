...
namespace Kassiopeia
{
    ...
    void KSTrajTermSynchrotron::Differentiate(double /*aTime*/, const KSTrajExactParticle& aParticle, KSTrajExactDerivative& aDerivative ) const
    {
        ...
        if ( !fOldMethode )
        {
            ...
            KThreeVector tFirstTerm = tVelocity * tElectricFieldGradient + tAcceleration.Cross( tMagneticField ) + tVelocity.Cross(tVelocity * tMagneticFieldGradient );

            KThreeVector tSecondTerm = Gamma * Gamma * Gamma / ( C * C) * ( tAcceleration.Cross( tVelocity.Cross( tLorentzForce ) ) );
    ...
    void KSTrajTermSynchrotron::Differentiate(double aTime, const KSTrajExactTrappedParticle& aParticle, KSTrajExactTrappedDerivative& aDerivative) const
    {
        if(aTime==0) return;
