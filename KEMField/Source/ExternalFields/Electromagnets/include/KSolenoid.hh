...
namespace KEMField
{
  ...
  public:
    ...
    void SetR(double r)                   { fP0[0] = fP1[0] = r; }
    void SetZ0(double z0)                 { fP0[2] = z0; }
    void SetZ1(double z1)                 { fP1[2] = z1; }
    void SetP0(const KPosition& p0)
    { SetZ0(p0[2]); SetR(sqrt(p0[0]*p0[0] + p0[1]*p0[1])); }
    void SetP1(const KPosition& p1)
    { SetZ1(p1[2]); SetR(sqrt(p1[0]*p1[0] + p1[1]*p1[1])); }

    double          GetCurrent() const { return fCurrent; }
    double GetCurrentDensity() const { return fCurrent/fabs(fP1[2]-fP0[2]); }
    double          GetR()      const { return fP0[0]; }
  ...
  Stream& operator>>(Stream& s,KSolenoid& c)
  {
  ...
  template <typename Stream>
  Stream& operator<<(Stream& s,const KSolenoid& c)
  {
