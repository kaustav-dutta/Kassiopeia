...
namespace KEMField{
  ...
  public:
    ...
    void SetR0(double r0)                 { fP0[0] = r0; }
    void SetZ0(double z0)                 { fP0[2] = z0; }
    void SetR1(double r1)                 { fP1[0] = r1; }
    void SetZ1(double z1)                 { fP1[2] = z1; }
    void SetP0(const KPosition& p0)         { SetZ0(p0[2]); SetR0(sqrt(p0[0]*p0[0] + p0[1]*p0[1])); }
    void SetP1(const KPosition& p1)         { SetZ1(p1[2]); SetR1(sqrt(p1[0]*p1[0] + p1[1]*p1[1])); }
    void SetIntegrationScale(unsigned int i)      { fIntegrationScale = i; }

    double          GetCurrent() const { return fCurrent; }
    double GetCurrentDensity() const { return fCurrent/fabs(fP1[0]-fP0[0])/fabs(fP1[2]-fP0[2]); }
    double          GetR0()      const { return fP0[0]; }
    double          GetZ0()      const { return fP0[2]; }
    double          GetR1()      const { return fP1[0]; }
    double          GetZ1()      const { return fP1[2]; }
  ...
  Stream& operator>>(Stream& s,KCoil& c)
  {
  ...
  template <typename Stream>
  Stream& operator<<(Stream& s,const KCoil& c)
  {
