...
unsigned int
KFMIdentitySet::GetSize() const
{
    return fIDSet.size();
}

void
KFMIdentitySet::AddID(unsigned int id)
{
    fIDSet.push_back(id);
}
...
bool
KFMIdentitySet::IsPresent(unsigned int id) const
{
    return (0 <= FindID(id));
}
...
void
KFMIdentitySet::SetIDs(const std::vector<unsigned int>* fill)
{
    fIDSet.clear();
...
void
KFMIdentitySet::GetIDs(std::vector<unsigned int>* fill) const
{
    fill->clear();
