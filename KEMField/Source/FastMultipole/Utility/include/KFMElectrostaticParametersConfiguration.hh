...
class KFMElectrostaticParametersConfiguration: public KSAInputOutputObject
{
    public:
        ...
        int GetStrategy() const {return fStrategy;};
        void SetStrategy(const int& n){fStrategy = n;};
        ...
        void SetInsertionRatio(const double& d){fInsertionRatio = d;};

        int GetUseCaching() const {return fUseCaching;};
        void SetUseCaching(const int& r){fUseCaching = r;};
        ...
        int GetAllowedNumber() const {return fAllowedNumber;};
        void SetAllowedNumber(const int& r){fAllowedNumber = r;};

        double GetAllowedFraction() const {return fAllowedFraction;};
        void SetAllowedFraction(const double& r){fAllowedFraction = r;};

        double GetBiasDegree() const {return fBiasDegree;};
        void SetBiasDegree(const double& r){fBiasDegree = r;};
        ...
        KFMElectrostaticParameters GetParameters() const
        {
            ...
            if(fUseRegionEstimation != 0){params.use_region_estimation = true;}

            params.use_caching = false;
            if(fUseCaching != 0){params.use_caching = true;}
        ...
        virtual std::string ClassName() const {return std::string("KFMElectrostaticParametersConfiguration");};
