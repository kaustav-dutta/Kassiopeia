...
bool
KFMPinchonGMatrixCalculator::ComputeGX(kfm_matrix* G) const
{
    if(CheckMatrixDim(G))
    {
...
bool
KFMPinchonGMatrixCalculator::ComputeGY(kfm_matrix* G) const
{
...
bool
KFMPinchonGMatrixCalculator::ComputeGZ(kfm_matrix* G) const
{
    if(CheckMatrixDim(G))
    {
        kfm_matrix_set_zero(G);
...
bool
KFMPinchonGMatrixCalculator::ComputeGZHat(kfm_matrix* G) const
{
...
bool
KFMPinchonGMatrixCalculator::ComputeGZHatInverse(kfm_matrix* G) const
{
    if(CheckHatMatrixDim(G))
    {
        kfm_matrix_set_zero(G);
...
bool
KFMPinchonGMatrixCalculator::CheckMatrixDim(const kfm_matrix* G) const
{
    unsigned int nrows = (2*fDegree + 3);
    unsigned int ncolumns = (2*fDegree + 1);

    if(nrows != G->size1){return false;};
    if(ncolumns != G->size2){return false;};

    return true;
}

bool
KFMPinchonGMatrixCalculator::CheckHatMatrixDim(const kfm_matrix* G) const
{
