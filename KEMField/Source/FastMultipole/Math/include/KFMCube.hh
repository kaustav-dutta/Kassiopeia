...
class KFMCube: public KSAFixedSizeInputOutputObject
{
    public:

        KFMCube(){ for(unsigned int i=0; i<NDIM+1; i++){ fData[i] = 0.0; } };
        ...
        void SetCenter(const double* center){ for(unsigned int i=0; i<NDIM; i++){fData[i] = center[i];} }
        void SetCenter(const KFMPoint<NDIM>& center){ for(unsigned int i=0; i<NDIM; i++){fData[i] = center[i]; } }

        //geometric property retrieval
        KFMPoint<NDIM> GetCenter() const {return KFMPoint<NDIM>(fData);};
        void GetCenter(double* center) const { for(unsigned int i=0; i<NDIM; i++){center[i] = fData[i]; } }
        ...
        inline KFMCube& operator= (const KFMCube& rhs)
        {
            if(&rhs != this)
            {
                for(unsigned int i=0; i<NDIM+1; i++){fData[i] = rhs.fData[i];}
            }
            return *this;
        }
...
template<unsigned int NDIM>
inline double& KFMCube<NDIM>::operator[](unsigned int i)
{
    return fData[i];
}

template<unsigned int NDIM>
inline const double& KFMCube<NDIM>::operator[](unsigned int i) const
{
    return fData[i];
}
...
Stream& operator>>(Stream& s, KFMCube<3>& aData)
{
...
template <typename Stream>
Stream& operator<<(Stream& s,const KFMCube<3>& aData)
{
