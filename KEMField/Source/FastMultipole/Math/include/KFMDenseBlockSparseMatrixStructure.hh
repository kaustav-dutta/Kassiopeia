...
class KFMDenseBlockSparseMatrixStructure: public KSAInputOutputObject
{
    public:
        ...
        void SetDimension(const size_t& dim){fDimension = dim;};
        size_t GetDimension() const {return fDimension;};

        void SetMaxMatrixElementBufferSize(const size_t& mx_element_buff_size){fMaxMatrixElementBufferSize = mx_element_buff_size;};
        size_t GetMaxMatrixElementBufferSize() const {return fMaxMatrixElementBufferSize;};

        void SetMaxIndexBufferSize(const size_t& index_buff_size ){fMaxIndexBufferSize = index_buff_size;};
        size_t GetMaxIndexBufferSize() const {return fMaxIndexBufferSize;};

        void SetRowsSizes(const std::vector<size_t>* row_sizes){fRowSizes = *row_sizes;};
        const std::vector<size_t>* GetRowSizes() const {return &fRowSizes;};

        void SetColumnSizes(const std::vector<size_t>* col_sizes){fColumnSizes = *col_sizes;};
        const std::vector<size_t>* GetColumnSizes() const {return &fColumnSizes;};
        ...
        size_t GetNBuffers() const {return fNBuffers;};
        size_t GetNBlocks() const {return fNBlocks;};
        size_t GetLargestBlockSize() const {return fLargestBlockSize;};
        size_t GetNTotalNonZeroElements() const {return fNTotalNonZeroElements;};
        size_t GetMaxRowWidth() const {return fMaxRowWidth;};
        size_t GetTotalNumberOfColumnIndices()  const {return fTotalNColumnIndices;};
        size_t GetTotalNumberOfRowIndices() const {return fTotalNRowIndices;};
        size_t GetLargestRowSize() const {return fLargestRowSize;};
        size_t GetLargestColumnSize() const {return fLargestColumnSize;};

        const std::vector<size_t>* GetNElements() const {return &fNElements;};
        const std::vector<size_t>* GetRowOffsets() const {return &fRowIndexOffsets;};
        const std::vector<size_t>* GetColumnOffsets() const {return &fColumnIndexOffsets;};
        const std::vector<size_t>* GetMatrixElementOffsets() const {return &fMatrixElementOffsets;};

        size_t GetMaxNumberOfBlocksInAnyBuffer() const {return fBufferMaxBlocks;};
        size_t GetMaxNumberOfColumnIndicesInAnyBuffer() const {return fBufferMaxColumnIndices;};
        size_t GetMaxNumberOfRowIndicesInAnyBuffer() const {return fBufferMaxRowIndices;};
        size_t GetMaxNumberOfElementsInAnyBuffer() const {return fBufferMaxElements;};
        size_t GetBufferStartBlockID(size_t buffer_id) const {return fBufferStartBlocks[buffer_id];};
        size_t GetBufferNumberOfBlocks(size_t buffer_id) const {return fNBlocksInBuffer[buffer_id];};
        size_t GetBufferMatrixElementSize(size_t buffer_id) const {return fMatrixElementBufferSize[buffer_id];};
        size_t GetBufferRowIndexSize(size_t buffer_id) const {return fRowIndexBufferSize[buffer_id];};
        size_t GetBufferColumnIndexSize(size_t buffer_id) const {return fColumnIndexBufferSize[buffer_id];};
    ...
    protected:
        ...
        std::vector< size_t > fRowIndexOffsets; //the offset from buffer start required to access the row indices of the each block
        std::vector< size_t > fColumnIndexOffsets; //the offset from buffer start required to access the column indices of the each block
        std::vector< size_t > fMatrixElementOffsets; //the offset from buffer start required to access the matrix elements of the each block
