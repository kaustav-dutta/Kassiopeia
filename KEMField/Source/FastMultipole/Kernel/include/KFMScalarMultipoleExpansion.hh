...
class KFMScalarMultipoleExpansion: public KFMScalarMomentExpansion, public KSAFixedSizeInputOutputObject
{
    public:
        ...
        std::vector<double>* GetRealMoments(){return &fMomentsReal;};
        std::vector<double>* GetImaginaryMoments(){return &fMomentsImag;};
        const std::vector<double>* GetRealMoments() const {return &fMomentsReal;};
        const std::vector<double>* GetImaginaryMoments() const {return &fMomentsImag;};
        void GetRealMoments( std::vector<double>* real_mom) const {*real_mom = fMomentsReal;};
        void GetImaginaryMoments( std::vector<double>* imag_mom) const {*imag_mom = fMomentsImag;};
        ...
        static inline int RealBasisIndex(int l, int m){return (l*(l+1))/2 + m;}; //real basis multipole index
        ...
        void PrintMoments() const
        {
            ...
            for(int l=0; l<=degree; l++)
            {
                for(int m=0; m<=l; m++)
                {
                    ...
                    kfmout<<"M("<<l<<", "<<m<<") = ("<<fMomentsReal[si]<<", "<<fMomentsImag[si]<<")"<<kfmendl;
                }
            }
        }
...
inline KFMScalarMultipoleExpansion&
KFMScalarMultipoleExpansion::operator=(const KFMScalarMultipoleExpansion& rhs)
{
    if(this != &rhs)
    {
...
inline KFMScalarMultipoleExpansion operator+( const KFMScalarMultipoleExpansion& left, const KFMScalarMultipoleExpansion& right )
{
...
inline KFMScalarMultipoleExpansion& operator+=(KFMScalarMultipoleExpansion& left, const KFMScalarMultipoleExpansion& right )
{
...
Stream& operator>>(Stream& s, KFMScalarMultipoleExpansion& aData)
{
...
template <typename Stream>
Stream& operator<<(Stream& s, const KFMScalarMultipoleExpansion& aData)
{
