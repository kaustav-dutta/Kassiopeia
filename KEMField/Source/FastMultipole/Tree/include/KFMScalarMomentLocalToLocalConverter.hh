...
class KFMScalarMomentLocalToLocalConverter: public KFMNodeActor< KFMNode<ObjectTypeList> >
{
    public:
        ...
        void SetNumberOfTermsInSeries(unsigned int n_terms)
        {
            ...
            delete[] fCBSI; fCBSI = new unsigned int[fNTerms];
            delete[] fRBSI; fRBSI = new unsigned int[fNTerms];
            ...
            if( node != nullptr && node->HasChildren() && node->GetLevel() != 0 )
            {
                ...
                if(mom != nullptr)
                {
                    ...
                    if(fIsScaleInvariant)
                    {
                        ...
                        {
                            fPtrLocalCoeffSource[si]  = (fParentsLocalCoeff[si])*(fKernelResponse->GetKernel()->GetSourceScaleFactor(si, scale));
                        }
                    }
                    ...
                    for(int j=0; j <= fDegree; j++)
                    {
                        for(int k=0; k <= j; k++)
                        {
                            ...
                            if(fIsScaleInvariant)
                            {
                                ...
                                fScalarMultCalc->SetScalarMultiplicationFactor(fKernelResponse->GetKernel()->GetTargetScaleFactor(tsi, scale));
    ...
    protected:
        ...
        void AllocateArrays()
        {
            ...
            fL2LCoeff =
            new KFMArrayWrapper<std::complex<double>, SpatialNDIM + 2>(fPtrL2LCoeff, fDimensionSize);
            ...
            fTempOut =
            new KFMArrayWrapper<std::complex<double>, SpatialNDIM>(fPtrTempOut, &(fDimensionSize[2]) );
        ...
        void FillParentCoefficientArray()
        {
            ...
            for(unsigned int i=0; i<fNTerms; i++)
            {
                ...
                for(unsigned int j=0; j<fTotalSpatialSize; j++)
                {
                    ...
                    for(unsigned int k=0; k<SpatialNDIM+1; k++){ index[k] = index_t[k];};
