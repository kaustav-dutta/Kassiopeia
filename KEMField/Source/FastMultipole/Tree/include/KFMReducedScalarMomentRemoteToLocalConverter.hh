...
template< typename ObjectTypeList, typename SourceScalarMomentType, typename TargetScalarMomentType, typename KernelType, unsigned int SpatialNDIM >
class KFMReducedScalarMomentRemoteToLocalConverter: public KFMNodeActor< KFMNode<ObjectTypeList> >
{
    public:
        ...
        KFMReducedScalarMomentRemoteToLocalConverter()
        {
            ...
            fKernelResponse = new KFMReducedKernelResponseArray<KernelType, true, SpatialNDIM>(); //true -> origin is source
            ...
            fOrigLocalCoeffCollector = new KFMReducedScalarMomentCollector<ObjectTypeList, TargetScalarMomentType, SpatialNDIM>();
    ...
    protected:
        ...
        void CreateAndAssociateArrayWrappers()
        {
            ...
            fAllMultipoles = new KFMArrayWrapper<std::complex<double>, SpatialNDIM + 1>(fPtrMultipoles, fSourceDimensionSize );
            fAllLocalCoeff = new KFMArrayWrapper<std::complex<double>, SpatialNDIM + 1>(fPtrLocalCoeff, fTargetDimensionSize );
            fAllOrigLocalCoeff = new KFMArrayWrapper<std::complex<double>, SpatialNDIM + 1>(fPtrOrigLocalCoeff, fTargetDimensionSize );
            fTempOutput = new KFMArrayWrapper<std::complex<double>, SpatialNDIM>(fPtrTempOutput, &(fTargetDimensionSize[1]) );
            ...
            for(unsigned int tsi=0; tsi<fNReducedTerms; tsi++)
            {
                ...
                fOrigLocalCoeff[tsi] = new KFMArrayWrapper<std::complex<double>, SpatialNDIM>(ptr, &(fTargetDimensionSize[1]) );
        ...
        virtual void ComputeM2LCoefficients()
        {
            ...
            for(unsigned int tsi=0; tsi<fNTerms; tsi++)
            {
                ...
                {
                    fPtrNormalization[ssi + tsi*fNTerms] = fNorm*(fKernelResponse->GetKernel()->GetNormalizationFactor(ssi, tsi));
                }
            }
        ...
        virtual void RescaleMultipoles(double scale_factor)
        {
            ...
            for(int n=0; n<=fDegree; n++)
            {
                for(int m=0; m<=n; m++)
                {
                    ...
                    fScalarMultCalc->SetScalarMultiplicationFactor(fScaleInvariantKernel->GetSourceScaleFactor(cssi, scale));
        ...
        virtual void RescaleLocalCoefficients(double scale_factor)
        {
            ...
            for(int j=0; j<=fDegree; j++)
            {
                for(int k=0; k<=j; k++)
                {
                    ...
                    std::complex<double> scale = std::complex<double>(scale_factor, 0.0);
                    //rescale the local coefficient constributions depending on the tree level
                    fScalarMultCalc->SetScalarMultiplicationFactor(fScaleInvariantKernel->GetTargetScaleFactor(ctsi, scale));
                ...
                if(fNeighbors[n] != nullptr)
                {
                    ...
                    for(unsigned int c = 0; c < fNeighbors[n]->GetNChildren(); c++)
                    {
                        ...
                        if(fChild != nullptr && KFMObjectRetriever<ObjectTypeList, TargetScalarMomentType>::GetNodeObject(fChild) != nullptr)
                        {
                            ...
                            //retrieve moments
                            TargetScalarMomentType* set = KFMObjectRetriever<ObjectTypeList, TargetScalarMomentType>::GetNodeObject(fChild);
