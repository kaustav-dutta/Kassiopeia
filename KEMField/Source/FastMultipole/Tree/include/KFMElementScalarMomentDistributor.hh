...
class KFMElementScalarMomentDistributor
{
    public:
        ...
        void SetBatchCalculator(KFMElementMomentBatchCalculator* batchCalc){fBatchCalc = batchCalc;};

        void SetElementIDList(const std::vector<unsigned int>* elementIDs){fElementIDList = elementIDs;};

        void SetNodeList(const std::vector< KFMNode<ObjectTypeList>* >* nodeList){fNodeList = nodeList;};

        void SetOriginList(const std::vector< KFMPoint<SpatialNDIM> >* origins){fOriginList = origins;};
        ...
        void ProcessAndDistributeMoments()
        {
            ...
            {
                //warning
                kfmout <<"KFMElementScalarMomentDistributor::ProcessAndDistributeMoments: Dimension of batch calculator does not equal "<<SpatialNDIM<<". Aborting."<<std::endl;
            }
        }
