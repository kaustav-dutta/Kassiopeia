...
class KFMSubdivisionConditionBalanced: public KFMSubdivisionCondition<NDIM, ObjectTypeList>
{
    public:
        ...
        void SetDiskWeight(double alpha){fDiskWeight = alpha;};
        void SetRamWeight(double alpha){fRamWeight = alpha;};
        void SetFFTWeight(double beta){fFFTWeight = beta;};
            ...
            if(level < max_depth)
            {
                ...
                if(bball_list->GetSize() != 0)
                {
                    ...
                    if( fSparseMatrixMemoryUse > KFMDenseBlockSparseMatrix<double>::GetSuggestedMatrixElementBufferSize() ||
                        mx_size > KFMDenseBlockSparseMatrix<double>::GetSuggestedMatrixElementBufferSize()/((double)total_size) )
                    {
