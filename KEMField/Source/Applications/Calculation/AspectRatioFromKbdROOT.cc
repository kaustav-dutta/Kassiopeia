...
void GetMinMaxAspectRatio( double* retValues, unsigned int containerSize, std::vector<KThreeVector> &shapeTypes, std::vector<double> &arValues )
{
    ...
    for( unsigned int i=0; i<containerSize; i++ ) {
        if( shapeTypes[i].X() == 1. ) { // triangle
            ...
            if(aspectRatioTri < aspectRatioTriMin) aspectRatioTriMin=aspectRatioTri;
            if(aspectRatioTri > aspectRatioTriMax) aspectRatioTriMax=aspectRatioTri;
        ...
        if( shapeTypes[i].Y() == 1. ) { // rectangle
            ...
            if(aspectRatioRect < aspectRatioRectMin) aspectRatioRectMin=aspectRatioRect;
            if(aspectRatioRect > aspectRatioRectMax) aspectRatioRectMax=aspectRatioRect;
        ...
        if( shapeTypes[i].Z() == 1. ) { // line segment
            ...
            if(aspectRatioLine < aspectRatioLineMin) aspectRatioLineMin=aspectRatioLine;

            if(aspectRatioLine < aspectRatioLineMin) aspectRatioLineMin=aspectRatioLine;
            if(aspectRatioLine > aspectRatioLineMax) aspectRatioLineMax=aspectRatioLine;
    ...
    KEMField::cout << "Min. aspect ratio for triangles: " << aspectRatioTriMin << ", max: " << aspectRatioTriMax << KEMField::endl;
    KEMField::cout << "Min. aspect ratio for rectangles: " << aspectRatioRectMin << ", max: " << aspectRatioRectMax << KEMField::endl;
    KEMField::cout << "Min. aspect ratio for line segments: " << aspectRatioLineMin << ", max: " << aspectRatioLineMax << KEMField::endl;
...
int main(int argc, char* argv[])
{
    ...
    if( countShapeTypes[0]>0 ) data[0]->Write("arTri");
    if( countShapeTypes[1]>0 ) data[1]->Write("arRect");
    if( countShapeTypes[2]>0 ) data[2]->Write("arLine");
