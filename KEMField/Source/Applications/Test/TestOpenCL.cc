...
namespace KEMField
{
  ...
  void KOpenCLInterface::SetGPU(unsigned int i)
  {
    ...
    {
      std::stringstream s;
      s << "Cannot set GPU device to ID # "<<i<<", since there are only "<<fDevices.size()<<" devices available.";
    ...
    {
      std::stringstream s;
      s << "Cannot set GPU device to ID # "<<i<<", since it does not support double precision (and this program was built with double precision enabled).";
      std::cout<<s.str()<<std::endl;
      return;
    }
...
int main(int argc, char** argv)
{
  ...
  std::cout << "---" << std::endl;
  std::cout << "Available GPUs: KOpenCLInterface::GetInstance()->GetDevices().size()         = " << KOpenCLInterface::GetInstance()->GetDevices().size() << std::endl;
  std::cout << "Total number of MPI processes: KMPIInterface::GetInstance()->GetNProcesses() = " << KMPIInterface::GetInstance()->GetNProcesses() << std::endl;
  ...
  KOpenCLInterface::GetInstance()->
    SetGPU(KMPIInterface::GetInstance()->GetProcess());
  ...
  catch (cl::Error& error)
  {
    ...
    std::cerr<<"Build Status: "<<program.getBuildInfo<CL_PROGRAM_BUILD_STATUS>(KOpenCLInterface::GetInstance()->GetDevice())<<std::endl;
    std::cerr<<"Build Options:\t"<<program.getBuildInfo<CL_PROGRAM_BUILD_OPTIONS>(KOpenCLInterface::GetInstance()->GetDevice())<<std::endl;
    std::cerr<<"Build Log:\t "<<program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(KOpenCLInterface::GetInstance()->GetDevice());
    return 0;
  }
  ...
  cl::Buffer* bufferIn;
  cl::Buffer* bufferOut;
  bufferIn  = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(), CL_MEM_READ_ONLY,DATA_SIZE*sizeof(double));
  bufferOut = new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(), CL_MEM_WRITE_ONLY,DATA_SIZE*sizeof(double));
  ...
  KOpenCLInterface::GetInstance()->GetQueue().enqueueWriteBuffer(*bufferIn,CL_TRUE,0,DATA_SIZE*sizeof(double),arcSinhInputs);
  ...
  cl::Event event;
  KOpenCLInterface::GetInstance()->GetQueue().enqueueReadBuffer(*bufferOut,CL_TRUE,0,DATA_SIZE*sizeof(double),arcSinhOutputs,NULL,&event);

  event.wait();

  for (unsigned int i=0;i<DATA_SIZE;i++)
  {
    std::cout<<"Process no. " << KMPIInterface::GetInstance()->GetProcess() + 1 << " : arcsinh(sinh("<<(double)i/DATA_SIZE<<")) = "<<arcSinhOutputs[i]<<std::endl;
  }
