...
namespace KEMField
{
  ...
  class KGaussSeidel_OpenCL : public KOpenCLAction
  {
  public:
  ...
  template <typename ValueType>
  KGaussSeidel_OpenCL<ValueType>::KGaussSeidel_OpenCL(const Matrix& A, Vector& x, const Vector& b) : KOpenCLAction((dynamic_cast<const KOpenCLAction&>(A)).GetData()), fA(A), fX(x), fB(b), fInitializeVectorApproximationKernel(NULL), fFindResidualKernel(NULL), fFindResidualNormKernel(NULL), fCompleteResidualNormalizationKernel(NULL), fIncrementIndexKernel(NULL), fComputeCorrectionKernel(NULL), fUpdateSolutionApproximationKernel(NULL), fUpdateVectorApproximationKernel(NULL), fBufferResidual(NULL), fBufferB_iterative(NULL), fBufferCorrection(NULL), fIndex(NULL), fBufferPartialResidualNorm(NULL), fBufferResidualNorm(NULL), fBufferNWarps(NULL), fBufferCounter(NULL), fCLResidual(NULL), fCLB_iterative(NULL), fCLCorrection(NULL), fCLIndex(NULL), fCLPartialResidualNorm(NULL), fCLResidualNorm(NULL), fCLNWarps(NULL), fCLCounter(NULL), fReadResidual(false)
  {
    KOpenCLAction::Initialize();
  }

  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::ConstructOpenCLKernels() const
  {
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::AssignBuffers() const
  {
    ...
    fBufferCorrection =new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
				 CL_MEM_READ_WRITE,
				 sizeof(CL_TYPE));
    fIndex =new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
				 CL_MEM_READ_WRITE,
				 sizeof(cl_int));
    ...
    KOpenCLInterface::GetInstance()->
    KOpenCLInterface::GetInstance()->
      GetQueue().enqueueWriteBuffer(*fIndex,
				    CL_TRUE,
				    0,
				    sizeof(cl_int),
				    fCLIndex);
  ...
  template <typename ValueType>
  KGaussSeidel_OpenCL<ValueType>::~KGaussSeidel_OpenCL()
  {
    ...
    if (fIncrementIndexKernel) delete fIncrementIndexKernel;
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::Initialize()
  {
    if (!fReadResidual)
    {
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::FindResidual()
  {
    try
    {
    KOpenCLInterface::GetInstance()->
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::FindResidualNorm(double& residualNorm)
  {
    KOpenCLInterface::GetInstance()->
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::CompleteResidualNormalization(double& residualNorm)
  {
    KOpenCLInterface::GetInstance()->
      GetQueue().enqueueNDRangeKernel(*fCompleteResidualNormalizationKernel,
				      cl::NullRange,
				      *fRangeOne,
				      *fRangeOne);
    cl::Event event;
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::IncrementIndex()
  {
    try
    {
    KOpenCLInterface::GetInstance()->
      GetQueue().enqueueNDRangeKernel(*fIncrementIndexKernel,
				      cl::NullRange,
				      *fRangeOne,
				      *fRangeOne);
    }
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::ComputeCorrection()
  {
    KOpenCLInterface::GetInstance()->
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::UpdateSolutionApproximation()
  {
    KOpenCLInterface::GetInstance()->
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::UpdateVectorApproximation()
  {
    KOpenCLInterface::GetInstance()->
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::CoalesceData()
  {
    dynamic_cast<KOpenCLSurfaceContainer&>(fData).ReadBasisData();
  }

  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::Finalize()
  {
    dynamic_cast<KOpenCLSurfaceContainer&>(fData).ReadBasisData();
  }

  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::SetResidualVector(const Vector& v)
  {
    fReadResidual = true;
  ...
  template <typename ValueType>
  void KGaussSeidel_OpenCL<ValueType>::GetResidualVector(Vector& v) const
  {
    cl::Event event;
    KOpenCLInterface::GetInstance()->GetQueue().
