...
namespace KEMField
{
  ...
  class KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL> :
  ...
  template <class Integrator>
  KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::
  KIntegratingFieldSolver(KOpenCLSurfaceContainer& container,
			  Integrator& integrator) :
  ...
  ~KIntegratingFieldSolver()
  {
    ...
    if (fElectricFieldAndPotentialKernel) delete fElectricFieldAndPotentialKernel;
    if (fSubsetPotentialKernel) delete fSubsetPotentialKernel;
    if (fSubsetElectricFieldKernel) delete fSubsetElectricFieldKernel;
    if (fSubsetElectricFieldAndPotentialKernel) delete fSubsetElectricFieldAndPotentialKernel;
    ...
    if (fCLElectricFieldAndPotential) delete fCLElectricFieldAndPotential;
  ...
  template <class Integrator>
  void KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::ConstructOpenCLKernels() const
  {
  ...
  template <class Integrator>
  void KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::AssignBuffers() const
  {
    ...
    fBufferElectricFieldAndPotential =
      new cl::Buffer(KOpenCLInterface::GetInstance()->GetContext(),
		     CL_MEM_WRITE_ONLY,
		     fNWorkgroups * sizeof(CL_TYPE4));
    ...
    KOpenCLInterface::GetInstance()->GetQueue().
      enqueueWriteBuffer(*fBufferElectricFieldAndPotential,
			 CL_TRUE,
			 0,
			 fNWorkgroups*sizeof(CL_TYPE4),
  ...
  std::pair<KThreeVector,double> KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::ElectricFieldAndPotential(const KPosition& aPosition) const
  {
    ...
    KOpenCLInterface::GetInstance()->GetQueue().
      enqueueNDRangeKernel(*fElectricFieldAndPotentialKernel,
    			   cl::NullRange,
    			   *fGlobalRange,
    			   *fLocalRange);
    ...
    KOpenCLInterface::GetInstance()->GetQueue().
      enqueueReadBuffer(*fBufferElectricFieldAndPotential,
    			CL_TRUE,
    			0,
    			fNWorkgroups * sizeof(CL_TYPE4),
    			fCLElectricFieldAndPotential);
    ...
    template <class Integrator>
    double KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::Potential(const unsigned int* SurfaceIndexSet, unsigned int SetSize, const KPosition& aPosition) const
    {
        //evaluation point
        CL_TYPE P[3] = {aPosition[0],aPosition[1],aPosition[2]};
    ...
    std::pair<KThreeVector,double> KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::ElectricFieldAndPotential(const unsigned int* SurfaceIndexSet, unsigned int SetSize, const KPosition& aPosition) const
    {
        ...
        if(SetSize > fMinSubsetSize && SetSize <= fMaxSubsetSize)
        {
            ...
            KOpenCLInterface::GetInstance()->GetQueue().
            enqueueNDRangeKernel(*fSubsetElectricFieldAndPotentialKernel,
		                         cl::NullRange,
		                         global,
		                         local);
    ...
    template <class Integrator>
    void KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::GetReorderedSubsetIndices(const unsigned int* sorted_container_ids, unsigned int* normal_container_ids, unsigned int size) const
    {
    ...
    template <class Integrator>
    void KIntegratingFieldSolver<Integrator, ElectrostaticOpenCL>::DispatchPotential(const unsigned int* SurfaceIndexSet, unsigned int SetSize, const KPosition& aPosition) const
    {
    ...
    template <class Integrator>
    void KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::DispatchElectricField(const unsigned int* SurfaceIndexSet, unsigned int SetSize, const KPosition& aPosition) const
    {
        fCachedSurfaceIndexSet = SurfaceIndexSet;
        fCachedSubsetSize = SetSize;
        fCallDevice = false;
    ...
    template <class Integrator>
    void KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::DispatchElectricFieldAndPotential(const unsigned int* SurfaceIndexSet, unsigned int SetSize, const KPosition& aPosition) const
    {
        fCachedSurfaceIndexSet = SurfaceIndexSet;
        fCachedSubsetSize = SetSize;
        fCallDevice = false;
    ...
    template <class Integrator>
    double KIntegratingFieldSolver<Integrator,ElectrostaticOpenCL>::RetrievePotential() const
    {
        if(fCallDevice)
        {
            CL_TYPE potential = 0.;
