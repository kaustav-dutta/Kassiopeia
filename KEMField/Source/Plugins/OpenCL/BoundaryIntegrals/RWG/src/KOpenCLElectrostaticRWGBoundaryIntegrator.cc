#include "KOpenCLElectrostaticRWGBoundaryIntegrator.hh"
...
namespace KEMField
{
  KOpenCLElectrostaticRWGBoundaryIntegrator::KOpenCLElectrostaticRWGBoundaryIntegrator(KOpenCLSurfaceContainer& c) :
  ...
  KOpenCLElectrostaticRWGBoundaryIntegrator::~KOpenCLElectrostaticRWGBoundaryIntegrator()
  {
  ...
  void KOpenCLElectrostaticRWGBoundaryIntegrator::BoundaryVisitor::Visit(KDirichletBoundary& boundary)
  {
    fIsDirichlet = true;
    fPrefactor = 1.;
  ...
  void KOpenCLElectrostaticRWGBoundaryIntegrator::BoundaryVisitor::Visit(KNeumannBoundary& boundary)
  {
    fIsDirichlet = false;
  ...
  void KOpenCLElectrostaticRWGBoundaryIntegrator::BasisVisitor::Visit(KElectrostaticBasis& basis)
  {
    fBasisValue = &(basis.GetSolution());
  }

  KElectrostaticBasis::ValueType KOpenCLElectrostaticRWGBoundaryIntegrator::BoundaryIntegral(KSurfacePrimitive* source,KSurfacePrimitive* target,unsigned int)
  {
    fTarget = target;
  ...
  KElectrostaticBasis::ValueType KOpenCLElectrostaticRWGBoundaryIntegrator::BoundaryValue(KSurfacePrimitive* surface,unsigned int)
  {
    surface->Accept(fBoundaryVisitor);
    return fBoundaryVisitor.GetBoundaryValue();
  }

  KElectrostaticBasis::ValueType& KOpenCLElectrostaticRWGBoundaryIntegrator::BasisValue(KSurfacePrimitive* surface,unsigned int)
  {
    surface->Accept(fBasisVisitor);
    return fBasisVisitor.GetBasisValue();
  }

  void KOpenCLElectrostaticRWGBoundaryIntegrator::ConstructOpenCLKernels() const
  {
    ...
    clFile << KOpenCLInterface::GetInstance()->GetKernelPath() << "/kEMField_ElectrostaticRWGBoundaryIntegrals_kernel.cl";
  ...
  void KOpenCLElectrostaticRWGBoundaryIntegrator::AssignBuffers() const
  {
    KOpenCLBoundaryIntegrator<KElectrostaticBasis>::AssignBuffers();

    //std::cout << "RWG Potential             - Workgroup Size: " << fPhiKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>((KOpenCLInterface::GetInstance()->GetDevice())) << std::endl;
    ...
    //std::cout << "RWG Electric Field        - Workgroup Size: " << fEFieldKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>((KOpenCLInterface::GetInstance()->GetDevice())) << std::endl;
    ...
    //std::cout << "RWG Electric Field and Phi- Workgroup Size: " << fEFieldAndPhiKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>((KOpenCLInterface::GetInstance()->GetDevice())) << std::endl;
