...
namespace KEMField
{
  ...
  class KBoundaryIntegralMatrix<KOpenCLBoundaryIntegrator<BasisPolicy> > : public KSquareMatrix<typename BasisPolicy::ValueType>, public KOpenCLAction
  {
  public:
    typedef typename BasisPolicy::ValueType ValueType;
    friend class KOpenCLSurfaceContainer;

    KBoundaryIntegralMatrix(KOpenCLSurfaceContainer& c,
    ...
    KOpenCLBoundaryIntegrator<BasisPolicy>& GetIntegrator() const { return fIntegrator; }
  ...
  template <class BasisPolicy>
  KBoundaryIntegralMatrix<KOpenCLBoundaryIntegrator<BasisPolicy> >::KBoundaryIntegralMatrix(KOpenCLSurfaceContainer& c,KOpenCLBoundaryIntegrator<BasisPolicy>& integrator) :
  ...
  {
    if (fGetMatrixElementKernel) delete fGetMatrixElementKernel;

    if (fBufferIJ) delete fBufferIJ;
    if (fBufferValue) delete fBufferValue;
  }

  template <class BasisPolicy>
  const typename BasisPolicy::ValueType& KBoundaryIntegralMatrix<KOpenCLBoundaryIntegrator<BasisPolicy> >::operator()(unsigned int i,unsigned int j) const
  {
    cl_int ij[2];
    ...
    KOpenCLInterface::GetInstance()->GetQueue().
      enqueueNDRangeKernel(*fGetMatrixElementKernel,
			   cl::NullRange,
			   global,
			   local);
  ...
  void KBoundaryIntegralMatrix<KOpenCLBoundaryIntegrator<BasisPolicy> >::ConstructOpenCLKernels() const
  {
    ...
    // Make kernel
    fGetMatrixElementKernel =
      new cl::Kernel(program, "GetMatrixElement");

    // define fNLocal
    if (fNLocal == -1)
      fNLocal = fGetMatrixElementKernel->getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>((KOpenCLInterface::GetInstance()->GetDevice()));
  ...
  template <class BasisPolicy>
  void KBoundaryIntegralMatrix<KOpenCLBoundaryIntegrator<BasisPolicy> >::AssignBuffers() const
  {
