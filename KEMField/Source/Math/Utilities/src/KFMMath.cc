...
double
KFMMath::A_Coefficient(int upper, int lower)
{
...
double
KFMMath::ALP_nm(int n, int m, const double& x)
{
    ...
    else
    {
        ...
        for(int l = m+2; l <= n; l++)
        {
            ...
            blm = KFMSquareRootUtilities::SqrtIntegerRatio( (2*l + 1)*(l + m - 1)*(l - m - 1), (l - m)*(l + m)*(2*l - 3) );
...
void
KFMMath::ALP_nm_array(int n_max, const double& x, double* val)
{
...
void
KFMMath::ALP_nm_unormalized_array(int n_max, const double& x, double* val)
{
    val[0] = 1.0;
    if(n_max == 0){return;};
...
double
KFMMath::I_secn(int n, double lower_limit, double upper_limit)
{
    if(n <= 0)
    {
        return 0;
...
double
KFMMath::I_trig1(int n, double lower_limit, double upper_limit)
{
    if( n <= 0 )
    {
        return 0;
    ...
    return (1.0/( (double)n - 1.0) )*( std::pow( std::cos(upper_limit), -1*n + 1 ) - std::pow( std::cos(lower_limit), -1*n + 1)  );
...
double
KFMMath::I_cheb1(int l, int m, double lower_limit, double upper_limit)
{
    if( m < 0 || l < 0)
    {
        return std::numeric_limits<double>::quiet_NaN();
...
double
KFMMath::I_cheb2(int l, int m, double lower_limit, double upper_limit)
{
    if( m < 0 || l < 0)
    {
        return std::numeric_limits<double>::quiet_NaN();
...
void
KFMMath::K_norm_array(int l_max, double h, double* val)
{
    double hpow = h;
    double plm;
    double fac;
...
void
KFMMath::K_norm_array(int l_max, double h, double* plm, double* val)
{
    double hpow = h;
    double fac;
    int si;
...
void
KFMMath::SphericalHarmonic_Cart(int l, int m, const double* cartesian_coords, double* result)
{
...
void
KFMMath::RegularSolidHarmonic_Cart(int l, int m, const double* cartesian_coords, double* result)
{
...
void
KFMMath::IrregularSolidHarmonic_Cart(int l, int m, const double* cartesian_coords, double* result)
{
    KFMMath::SphericalHarmonic_Cart(l, m, cartesian_coords, result);
...
std::complex<double>
KFMMath::SphericalHarmonic_Sph(int l, int m, const double* spherical_coords)
{
...
std::complex<double>
KFMMath::SphericalHarmonic_Cart(int l, int m, const double* cartesian_coords)
{
    double cosTheta = KFMMath::CosTheta(cartesian_coords);
    double phi = KFMMath::Phi(cartesian_coords);
    double plm = KFMMath::ALP_nm(l, std::abs(m), cosTheta);
...
std::complex<double>
KFMMath::RegularSolidHarmonic_Cart(int l, int m, const double* cartesian_coords)
{
...
std::complex<double>
KFMMath::IrregularSolidHarmonic_Cart(int l, int m, const double* cartesian_coords)
{
    std::complex<double> val = KFMMath::SphericalHarmonic_Cart(l, m, cartesian_coords);
...
void
KFMMath::RegularSolidHarmonic_Cart_Array(int n_max, const double* cartesian_coords, std::complex<double>* result)
{
...
void
KFMMath::IrregularSolidHarmonic_Cart_Array(int n_max, const double* cartesian_coords, std::complex<double>* result)
{
    unsigned int max_size = (n_max*(n_max+1))/2 + n_max + 1;
