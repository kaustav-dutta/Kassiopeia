...
namespace KEMField
{
  ...
  public:
    ...
    void ComputeCoefficients() { ComputeCoefficients(-1); }

    ElementContainer& GetElementContainer() { return fElementContainer; }
    const ElementContainer& GetElementContainer() const { return fElementContainer; }
    const KEMCoordinateSystem& GetCoordinateSystem() const { return fCoordinateSystem; }
    SourcePointVector& GetCentralSourcePoints() { return fCentralSourcePoints; }
    const SourcePointVector& GetCentralSourcePoints() const { return fCentralSourcePoints; }
    SourcePointVector& GetRemoteSourcePoints() { return fRemoteSourcePoints; }
    const SourcePointVector& GetRemoteSourcePoints() const { return fRemoteSourcePoints; }
    ZonalHarmonicContainerVector& GetSubContainers() { return fSubContainers; }
    const ZonalHarmonicContainerVector& GetSubContainers() const { return fSubContainers; }
    KZonalHarmonicParameters& GetParameters() { return *fParameters; }
    const KZonalHarmonicParameters& GetParameters() const { return *fParameters; }
  ...
  private:
    ...
    template <typename Stream>
    friend Stream& operator<<(Stream& s,const KZonalHarmonicContainer<Basis>& c)
    {
  ...
  template <class Basis>
  KZonalHarmonicContainer<Basis>::KZonalHarmonicContainer(ElementContainer& elementContainer,KZonalHarmonicParameters* parameters) :
  ...
  template <class Basis>
  void KZonalHarmonicContainer<Basis>::ConstructSubContainers(int level)
  {
  ...
  template <class Basis>
  void KZonalHarmonicContainer<Basis>::ComputeCoefficients(int level)
  {
    if (fElementContainer.empty())
      return;

    KZHLegendreCoefficients::GetInstance()->InitializeLegendrePolynomialArrays((fParameters->GetNCentralCoefficients() > fParameters->GetNRemoteCoefficients() ? fParameters->GetNCentralCoefficients() : fParameters->GetNRemoteCoefficients()));
	...
	coefficientGenerator.
	  GenerateCentralSourcePointsByFractionalDistance(fCentralSourcePoints,
      ...
      {
	coefficientGenerator.
	  GenerateCentralSourcePointsByFixedDistance(fCentralSourcePoints,
						     fParameters->GetNCentralCoefficients(),
						     fParameters->GetCentralDeltaZ(),
						     z1,
						     z2);
	...
	z2 = fParameters->GetRemoteZ2();
      ...
      coefficientGenerator.
	GenerateRemoteSourcePoints(fRemoteSourcePoints,
