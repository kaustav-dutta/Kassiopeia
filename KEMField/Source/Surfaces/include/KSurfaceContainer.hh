...
*/
    ...
    public:
      ...
      typedef std::iterator<std::bidirectional_iterator_tag,
			    KSurfacePrimitive*>::value_type value_type;
      typedef std::iterator<std::bidirectional_iterator_tag,
			    KSurfacePrimitive*>::reference reference;
      typedef std::iterator<std::bidirectional_iterator_tag,
			    KSurfacePrimitive*>::pointer pointer;
    
      friend class KSurfaceContainer;

      iterator() {}

      iterator(const iterator& anIt) : fArrayIt(anIt.fArrayIt),
    				       fDataIt(anIt.fDataIt),
    				       fData(anIt.fData) {}
      iterator(KSurfaceArrayIt anArrayIt,
    	       KSurfaceDataCIt aDataIt,
	       SmartDataPointer aData) : fArrayIt(anArrayIt),
					 fDataIt(aDataIt),
					 fData(aData) {}

      iterator& operator=(const iterator& anIt)
      { fArrayIt = anIt.fArrayIt; fDataIt = anIt.fDataIt; fData = anIt.fData; return *this; }
      ...
      reference operator*() const { return *fArrayIt; }
      pointer operator->() const { return &(*fArrayIt); }
    ...
    unsigned int NumberOfSurfaceTypes() const { return fSurfaceData.size(); }
    ...
    KSurfacePrimitive* at(const unsigned int& i) const { return operator[](i); }
    unsigned int size() const;
    iterator begin() const;
    iterator end() const;
    bool empty() const { return fSurfaceData.size() == 0; }
    void clear();
    ...
    KSurfacePrimitive* operator[](unsigned int i) const;
    template <class BoundaryPolicy, class ShapePolicy>
    KSurfacePrimitive* at(unsigned int i) const
    ...
    iterator end() const;
    template <class Policy>
    bool empty() const { return this->size<Policy>() == 0; }
    template <class Policy>
    void clear();
  ...
  private:
    ...
    mutable SmartDataPointer fPartialSurfaceData[Length<KEMField::KBoundaryTypes>::value+1][Length<KEMField::KShapeTypes>::value+1];

    // Generalized streaming methods to facilitate recursive serialization (see
    // KFundamentalTypes.hh).

    template <typename Stream>
    friend Stream& operator>>(Stream& s,
			      KSurfaceContainer& aContainer)
    {
    ...
    template <typename Stream>
    friend Stream& operator<<(Stream& s,
			      const KSurfaceContainer& aContainer)
    {
  ...
  template <class BoundaryPolicy, class ShapePolicy>
  KSurfacePrimitive* KSurfaceContainer::operator[](unsigned int i) const
  {
  ...
  template <class BoundaryPolicy, class ShapePolicy>
  typename KSurfaceContainer::iterator KSurfaceContainer::begin() const
  {
  ...
  template <class BoundaryPolicy, class ShapePolicy>
  typename KSurfaceContainer::iterator KSurfaceContainer::end() const
  {
    typename KSurfaceContainer::iterator anIterator;
    anIterator.fData = GetSurfaceData<BoundaryPolicy,ShapePolicy>();
  ...
  {
    int basisPolicy =
      ((int)IndexOf<KBasisTypes,Policy>::value == -1 ?
       (int)Length<KBasisTypes>::value : (int)IndexOf<KBasisTypes,Policy>::value);
    int boundaryPolicy =
      ((int)IndexOf<KBoundaryTypes,Policy>::value == -1 ?
       (int)Length<KBoundaryTypes>::value : (int)IndexOf<KBoundaryTypes,Policy>::value);
    int shapePolicy =
      ((int)IndexOf<KShapeTypes,Policy>::value == -1 ?
       (int)Length<KShapeTypes>::value : (int)IndexOf<KShapeTypes,Policy>::value);
  ...
  template <typename Stream>
  Stream& operator>>(Stream& s,typename KSurfaceContainer::KSurfaceArray& anArray)
  {
  ...
  template <typename Stream>
  Stream& operator<<(Stream& s,const typename KSurfaceContainer::KSurfaceArray& anArray)
  {
    if (anArray.size()==0)
      return s;
  ...
  template <typename Stream>
  Stream& operator>>(Stream& s,typename KSurfaceContainer::KSurfaceData& aData)
  {
  ...
  template <typename Stream>
  Stream& operator<<(Stream& s,const typename KSurfaceContainer::KSurfaceData& aData)
  {
