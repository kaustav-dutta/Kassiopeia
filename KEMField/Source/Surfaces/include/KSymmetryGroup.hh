...
*/
  ...
  class KSymmetryGroup : public KShape
  {
  public:
  ...
  public:
    ...
    ShapePolicy* at(unsigned int i) { return fElements.at(i); }
    const ShapePolicy* at(unsigned int i) const { return fElements.at(i); }

    ShapePolicy* operator[](unsigned int i) { return fElements[i]; }
    const ShapePolicy* operator[](unsigned int i) const { return fElements[i]; }

    ShapeIt begin() { return fElements.begin(); }
    ShapeCIt begin() const { return fElements.begin(); }
    ShapeIt end() { return fElements.end(); }
    ShapeCIt end() const { return fElements.end(); }

    ShapeArray& Elements() { return fElements;}
    ...
    bool IsOtherSymmetry() const { return fOther; }
  ...
  protected:
    ...
    friend Stream& operator>>(Stream& s,KSymmetryGroup<ShapePolicy>& y)
    {
    ...
    template <typename Stream>
    friend Stream& operator<<(Stream& s,const KSymmetryGroup<ShapePolicy>& y)
    {
  ...
  template <class ShapePolicy>
  ShapePolicy* KSymmetryGroup<ShapePolicy>::NewElement()
  {
    if (fElements.size()!=0)
      fOther = true;
  ...
  template <class ShapePolicy>
  ShapePolicy* KSymmetryGroup<ShapePolicy>::NewElementImpl()
  {
