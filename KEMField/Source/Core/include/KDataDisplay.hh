 ...
 */
  ...
  struct KDataDisplayType;

  struct KNullStream {};

  template <typename Type>
  struct KDataDisplayType<Type,KNullStream>
  {
    friend inline KDataDisplay<KNullStream>& operator<<(KDataDisplayType<Type,KNullStream>& d,const Type) { return d.Self(); }
  ...
  template <>
  class KDataDisplayOtherTypes<KNullStream>
  {
  public:
    KDataDisplayOtherTypes() {}
    virtual ~KDataDisplayOtherTypes() {}
  ...
  protected:
    ...
    std::string GetObjectName(Type2Type<Streamed>) {return Streamed::Name();}
    template <class Streamed>
    std::string GetObjectName(Type2Type<Streamed*>) {std::stringstream s; s<<Streamed::Name()<<"*";return s.str();}
    template <class Streamed>
    std::string GetObjectName(Type2Type<Streamed&>) {std::stringstream s;s<<Streamed::Name()<<"&";return s.str();}
    template <class Streamed>
    std::string GetObjectName(Type2Type<std::vector<Streamed> >) { std::stringstream s; s<<"(std::vector)["<<GetObjectName(Type2Type<Streamed>())<<"]"; return s.str(); }
    template <class Streamed>
    std::string GetObjectName(Type2Type<std::vector<Streamed>*>) { std::stringstream s; s<<"(std::vector)["<<GetObjectName(Type2Type<Streamed>())<<"]*"; return s.str(); }
    template <class Streamed>
    std::string GetObjectName(Type2Type<std::vector<Streamed>&>) { std::stringstream s; s<<"(std::vector)["<<GetObjectName(Type2Type<Streamed>())<<"]&"; return s.str(); }
  ...
  public:
    ...
    template <class Streamed>
    void PreStreamOutActionForNamed(const Streamed&,Int2Type<false>) {}
    template <class Streamed>
    void PostStreamOutActionForNamed(const Streamed&,Int2Type<false>) {}

    template <class Streamed>
    void PreStreamOutActionForNamed(const Streamed& s,Int2Type<true>);
    template <class Streamed>
    void PostStreamOutActionForNamed(const Streamed&,Int2Type<true>);
  ...
  public:
    template <typename Type>    
    friend KDataDisplay<Stream>& operator <<(KDataDisplay<Stream>& s,
					     const std::complex<Type>& c)
    {
      std::stringstream ss;ss<<c;s<<ss.str();return s;
    }
  ...
  template <class Stream>
  static inline KDataDisplay<Stream>& endl(KDataDisplay<Stream>& myDisplay)
  {
  ...
  template <class Stream>
  template <class Streamed>
  void KDataDisplay<Stream>::PreStreamOutActionForNamed(const Streamed&,
							Int2Type<true>)
  {
    if (!fVerbose)
      return;
  ...
  template <class Stream>
  template <class Streamed>
  void KDataDisplay<Stream>::PostStreamOutActionForNamed(const Streamed&,
							 Int2Type<true>)
  {
    if (!fVerbose)
      return;
    fLevel--;
    Reset(fLevel);
    for (int i=0;i<fLevel;i++) fStream << fSeparator;
  ...
  template <class Stream>
  std::string KDataDisplay<Stream>::fSeparator = "    ";

  template <class Stream>
  KDataDisplay<Stream>::KDataDisplay() : KDataDisplayOtherTypes<Stream>(),
  ...
  template <class Stream>
  void KDataDisplay<Stream>::Reset(int i)
  {
    if (i==-1)
