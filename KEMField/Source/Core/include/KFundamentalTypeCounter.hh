...
*/
  ...
  class KFundamentalTypeCounterType
  {
  public:
  ...
  typedef KGenScatterHierarchy<KEMField::FundamentalTypes,
			       KFundamentalTypeCounterType>
  KFundamentalTypeCounterTypes;
  ...
  public:
    ...
    unsigned int NumberOfTypes() const { return fTypesCounter; }

    template <typename Type>
    unsigned int NumberOfType() { return fTypeCounter[IndexOf<KEMField::FundamentalTypes,Type>::value]; }

    void Reset() { fTypesCounter = 0; for (unsigned int i=0;i<Length<KEMField::FundamentalTypes>::value;i++) fTypeCounter[i] = 0; }
