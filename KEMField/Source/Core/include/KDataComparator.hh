...
namespace KEMField
{
  ...
  typedef KGenScatterHierarchy<KEMField::FundamentalTypes,KDataComparatorType>
  KDataComparatorFundamentalTypes;
  ...
  public:
    ...
    template <class Streamed>
    void PostStreamInAction(const Streamed&) {}
    template <class Streamed>
    void PreStreamOutAction(const Streamed&) {}
    template <class Streamed>
    void PostStreamOutAction(const Streamed&) {}

    template <class StreamedLHS,class StreamedRHS>
    bool Compare(const StreamedLHS&,
		 const StreamedRHS&,
		 double) const { return false; }

    template <class Streamed>
    bool Compare(const Streamed& lhs,
		 const Streamed& rhs,
		 double tol=1.e-14);

    template <class Streamed>
    bool Compare(const std::vector<Streamed>& lhs,
		 const std::vector<Streamed>& rhs,
		 double tol=1.e-14);
  ...
  class KDataComparison
  {
  public:
    static void Initialize(KDataComparator& comparator)
    {
      (static_cast < KDataComparatorType < typename KEMField::TypeAt<KEMField::FundamentalTypes,typeID>::Result > &>(comparator)).Initialize();
      return KDataComparison<typeID+1>::Initialize(comparator);
    }

    static bool Result(KDataComparator& comparator)
    {
      if ((static_cast < KDataComparatorType < typename KEMField::TypeAt<KEMField::FundamentalTypes,typeID>::Result > &>(comparator)).Comparison())
  ...
  {
  public:
    static void Initialize(KDataComparator&) { return; }
    static bool Result(KDataComparator&) { return true; }
  };

  template <class Streamed>
  bool KDataComparator::Compare(const Streamed& lhs,
				const Streamed& rhs,
				double tol)
  {
  ...
  template <class Streamed>
  bool KDataComparator::Compare(const std::vector<Streamed>& lhs,
				const std::vector<Streamed>& rhs,
				double tol)
  {
  ...
  template <class Object>
  bool operator== (const typename EnableIf<IsNamed<Object>::Is,Object>::type& lhs, const typename EnableIf<IsNamed<Object>::Is,Object>::type& rhs)
  {
    KDataComparator dC;
    return dC.Compare(lhs,rhs);
  }
