...
{
  bool 
  CompareClassOrdering_weak(KMetadataStreamer::ClassOrdering::value_type i,
  ...
  void KMetadataStreamer::open(const std::string& fileName,
  ...
  std::string KMetadataStreamer::StringifyMetadata()
  {
	...
	{
	  if (it == it2) continue;
	  if (CompareClassOrdering_weak(*it,*it2) &&
	      !(CompareClassOrdering(*it,*it2)))
	  {
	    int tmp = it->first;
	    it->first = it2->first;
	    it2->first = tmp;
  ...
  {
    if (fHierarchy.data.size()==0) return;
