...
namespace KEMField
{
  ...
  typedef KGenScatterHierarchy<KEMField::FundamentalTypes,KMetadataStreamerType>
  KMetadataStreamerFundamentalTypes;
  ...
  public:
    ...
    std::string GetFileSuffix() const { return ".smd"; }
  ...
  protected:
    ...
    template <class Streamed>
    std::string GetName(Type2Type<std::vector<Streamed> >)
    {
      std::stringstream s;
    ...
    friend bool CompareClassOrdering_weak(ClassOrdering::value_type i,
					    ClassOrdering::value_type j);

    friend bool CompareClassOrdering(ClassOrdering::value_type i,
				       ClassOrdering::value_type j);
  ...
  template <class Streamed>
  void KMetadataStreamer::PreStreamOutActionForNamed(const Streamed&,
						     Int2Type<true>)
  {
    ...
    if (std::find(fData.completed.begin(),
		  fData.completed.end(),
		  className) != fData.completed.end())
  ...
  template <class Streamed>
  void KMetadataStreamer::PostStreamOutActionForNamed(const Streamed&,
						      Int2Type<true>)
  {
