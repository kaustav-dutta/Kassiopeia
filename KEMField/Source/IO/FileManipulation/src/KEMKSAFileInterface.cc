...
void
KEMKSAFileInterface::ReadKSAFile(KSAInputNode* node, string file_name, bool& result)
{
    result = false;
...
void
KEMKSAFileInterface::SaveKSAFile(KSAOutputNode* node, string file_name, bool& result, bool forceOverwrite)
{
        result = false;
        set<string> fileList = KEMFileInterface::GetInstance()->CompleteFileList();
        std::string full_file_name = KEMFileInterface::GetInstance()->ActiveDirectory() + "/" + file_name;
