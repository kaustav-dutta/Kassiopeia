...
*/
  ...
  template <>
  struct KMD5HashGeneratorType<float>
  {
...
#pragma GCC diagnostic pop
  ...
  template <>
  struct KMD5HashGeneratorType<double>
  {
...
#pragma GCC diagnostic pop
  ...
  typedef KGenScatterHierarchy<KEMField::FundamentalTypes,
			       KMD5HashGeneratorType>
  KMD5HashGeneratorFundamentalTypes;
  ...
  template <class Head,class Tail>
  void KMD5HashGenerator::Omit(KTypelist<Head,Tail>)
  {
    Omit(Type2Type<Head>());
    return Omit(Tail());
  }
  ...
  template <class X>
  std::string KMD5HashGenerator::GenerateHash(const X& x)
  {
    if (fMD5) delete fMD5;
    fMD5 = new MD5();
    *this << x;
  ...
  template <class Streamed>
  void KMD5HashGenerator::PostStreamOutAction(const Streamed&)
  {
    if (fOmitting)
    {
