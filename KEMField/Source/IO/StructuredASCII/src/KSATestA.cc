...
const char* KSATestA::GetName() const {return "KSATestA";}

void KSATestA::AddData(double data){fData.push_back(data);}
void KSATestA::ClearData(){fData.clear(); }
...
void KSATestA::SetData(const std::vector<double>* data){fData = *data;}

const KSATestB* KSATestA::GetB() const {return &fB;}
...
void KSATestA::DefineInputNode(KSAInputNode* node)
{
    if(node != nullptr)
    {
        ...
        complicated_node->AddChild<std::vector< KSATestB* >, KSATestD>( new KSAAssociatedAllocatedToVectorPointerObjectInputNode<  std::vector< KSATestB* > , KSATestD >( KSAClassName<KSATestD>::name() ) );
