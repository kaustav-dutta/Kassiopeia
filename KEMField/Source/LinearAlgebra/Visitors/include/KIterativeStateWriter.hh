...
namespace KEMField
{
  ...
  {
    KResidualThreshold() :
      fResidualThreshold( std::numeric_limits< double >::max() ),
      fGeometryHash( "" )
    {
    }
  ...
  Stream& operator>>( Stream& s, KResidualThreshold& r )
  {
  ...
  template< typename Stream >
  Stream& operator<<( Stream& s, const KResidualThreshold& r )
  {
  ...
  class KIterativeStateWriter : public KIterativeSolver<ValueType>::Visitor
  {
  public:
  ...
  template <typename ValueType>
  void KIterativeStateWriter<ValueType>::Initialize(KIterativeSolver<ValueType>& solver)
  {
    MPI_SINGLE_PROCESS
    {
  ...
  template <typename ValueType>
  void KIterativeStateWriter<ValueType>::Visit(KIterativeSolver<ValueType>& solver)
  {
    solver.CoalesceData();
    ...
    MPI_SINGLE_PROCESS
    {
      ...
      std::string activeNameRoot = KEMFileInterface::GetInstance()->GetActiveFileName().substr(0,KEMFileInterface::GetInstance()->GetActiveFileName().find_last_of("."));
      std::stringstream saveName;
      saveName << activeNameRoot
	       << "_"
	       << fSaveNameRoot
	       << "_"
	       << solver.Iteration()
  ...
  template <typename ValueType>
  void KIterativeStateWriter<ValueType>::Finalize(KIterativeSolver<ValueType>& solver)
  {
    solver.GetResidualVector(fResidualVector);
    fResidualVector.SetIteration(solver.Iteration());
