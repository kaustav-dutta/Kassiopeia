...
namespace KEMField
{
  ...
  class KMultiElementRobinHood_SingleThread
  {
  public:
    ...
    ~KMultiElementRobinHood_SingleThread() {}
  ...
  template <typename ValueType>
  const ValueType& KMultiElementRobinHood_SingleThread<ValueType>::SubMatrix::operator()(unsigned int i,unsigned int j) const
  {
  ...
  template <typename ValueType>
  KMultiElementRobinHood_SingleThread<ValueType>::KMultiElementRobinHood_SingleThread(const Matrix& A,Vector& x,const Vector& b) : fA(A), fX(x), fB(b), fSubspaceDimension(2), fSubA(A,fMaxResidualIndex), fSubResidual(fResidual,fMaxResidualIndex)
  {
  }

  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::Initialize()
  {
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::FindResidual()
  {
    for (unsigned int i=0;i<fB.Dimension();i++)
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::FindResidualNorm(double& residualNorm)
  {
    residualNorm = 0.;
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::IdentifyLargestResidualElements()
  {
    for (unsigned int i=0;i<fSubspaceDimension;i++)
      fMaxResidualIndex[i] = i;
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::ComputeCorrection()
  {
    fSubSolver.Solve(fSubA,fCorrection,fSubResidual);
  }

  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::UpdateSolutionApproximation()
  {
    for (unsigned int i=0;i<fSubspaceDimension;i++)
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::UpdateVectorApproximation()
  {
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::SetResidualVector(const Vector& v)
  {
  ...
  template <typename ValueType>
  void KMultiElementRobinHood_SingleThread<ValueType>::GetResidualVector(Vector& v) const
  {
    for (unsigned int i = 0;i<fResidual.Dimension();i++)
      v[i] = fResidual(i);
  }
