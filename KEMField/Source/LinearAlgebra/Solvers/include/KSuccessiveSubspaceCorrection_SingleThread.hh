...
namespace KEMField
{
  ...
  class KSuccessiveSubspaceCorrection_SingleThread
  {
  public:
  ...
  template <typename ValueType>
  const ValueType& KSuccessiveSubspaceCorrection_SingleThread<ValueType>::SubMatrix::operator()(unsigned int i,unsigned int j) const
  {
  ...
  template <typename ValueType>
  KSuccessiveSubspaceCorrection_SingleThread<ValueType>::KSuccessiveSubspaceCorrection_SingleThread(const Matrix& A,Vector& x,const Vector& b) : fA(A), fX(x), fB(b), fSubspaceDimension(2), fSubA(A,fResidualIndex), fSubResidual(fResidual,fResidualIndex)
  {
    fIndex = Dimension()-fSubspaceDimension-1;
  }

  template <typename ValueType>
  void KSuccessiveSubspaceCorrection_SingleThread<ValueType>::Initialize()
  {
  ...
  template <typename ValueType>
  void KSuccessiveSubspaceCorrection_SingleThread<ValueType>::FindResidual()
  {
    for (unsigned int i=0;i<fB.Dimension();i++)
  ...
  template <typename ValueType>
  void KSuccessiveSubspaceCorrection_SingleThread<ValueType>::IdentifyResidualElements()
  {
  ...
  template <typename ValueType>
  void KSuccessiveSubspaceCorrection_SingleThread<ValueType>::ComputeCorrection()
  {
    fSubSolver.Solve(fSubA,fCorrection,fSubResidual);
  }

  template <typename ValueType>
  void KSuccessiveSubspaceCorrection_SingleThread<ValueType>::UpdateSolutionApproximation()
  {
    for (unsigned int i=0;i<fSubspaceDimension;i++)
  ...
  template <typename ValueType>
  void KSuccessiveSubspaceCorrection_SingleThread<ValueType>::UpdateVectorApproximation()
  {
