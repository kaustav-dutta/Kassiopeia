...
KGradient KRampedMagneticField::MagneticGradientCore(
        const KPosition& aSamplePoint, const double& aSampleTime) const {
...
double KRampedMagneticField::GetModulationFactor(
...
double KRampedMagneticField::GetDerivModulationFactor(
        const double& aTime) const {
    double tLength = fRampUpDelay + fRampUpTime + fRampDownDelay + fRampDownTime;
...
//    assert( fRampDownDelay >= 0. ); ramping back can also start before t=0
    if (fRampingType == rtExponential ||
            fRampingType == rtInversion ||
            fRampingType == rtInversion2 ||
