...
void KExplicitSuperpositionCachedChargeDensitySolver::InitializeCore( KSurfaceContainer& container )
{
    ...
    {
        throw KEMSimpleException("must provide a set of scale factors and hash labels for explicit cached bem solution");
    }
    ...
    for(unsigned int n=0; n<fScaleFactors.size(); n++)
    {
        ...
        if(tSolution)
        {
            ...
            {
                cout << "adding scaled solution for geometry with name <" << fNames[n] << ">" << endl;
            ...
            {
                throw KEMSimpleException("problem dimension does not match that of geometry <" + fNames[n] + "> with hash label <" + fHashLabels[n] + ">.");
                break;
            }
    ...
    {
        throw KEMSimpleException("could not find needed set of cached bem solutions in directory <" + KEMFileInterface::GetInstance()->ActiveDirectory() + ">");
    }
}
