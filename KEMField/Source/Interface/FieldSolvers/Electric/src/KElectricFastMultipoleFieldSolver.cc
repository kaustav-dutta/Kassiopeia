...
void KElectricFastMultipoleFieldSolver::InitializeCore( KSurfaceContainer& container )
    {
        ...
        {
            #ifdef KEMFIELD_USE_OPENCL
            fFastMultipoleFieldSolverOpenCL = new KFMElectrostaticFastMultipoleFieldSolver_OpenCL(
...
void KElectricFastMultipoleFieldSolver::UseOpenCL( bool choice )
{
	if( choice == true )
	{
		...
		return;
#endif
		KEMField::cout << "cannot use opencl in fast multipole without kemfield being built with opencl, using defaults." << KEMField::endl;
	}
	fUseOpenCL = false;
	return;
}
